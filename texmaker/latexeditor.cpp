...
extern struct updateStruct updateStructureList(QTextDocument *doc, int blockpos,
  ...
  offset = tagStart;
  if ((tagStart != -1) && ((tagComment == -1) ||
                           ((tagComment != -1) && (tagStart <= tagComment)))) {
  ...
  s = text;
  tagStart =
      s.indexOf(QRegExp("\\\\" + struct_level3 + "\\*?[\\{\\[]"), tagEnd);
  tagEnd = -1;
  offset = tagStart;
  if ((tagStart != -1) && ((tagComment == -1) ||
                           ((tagComment != -1) && (tagStart <= tagComment)))) {
  ...
  if (!found) {
    ...
    s = text;
    tagStart =
        s.indexOf(QRegExp("\\\\" + struct_level4 + "\\*?[\\{\\[]"), tagEnd);
    tagEnd = -1;
    offset = tagStart;
    if ((tagStart != -1) &&
        ((tagComment == -1) ||
  ...
  if (!found) {
    ...
    s = text;
    tagStart =
        s.indexOf(QRegExp("\\\\" + struct_level5 + "\\*?[\\{\\[]"), tagEnd);
    tagEnd = -1;
    offset = tagStart;
    if ((tagStart != -1) &&
        ((tagComment == -1) ||
  ...
  if (!found) {
    ...
    s = text;
    tagStart =
        s.indexOf(QRegExp("\\\\" + struct_level1 + "\\*?[\\{\\[]"), tagEnd);
    tagEnd = -1;
    offset = tagStart;
    if ((tagStart != -1) &&
        ((tagComment == -1) ||
  ...
  if (!found) {
    ...
    s = text;
    tagStart =
        s.indexOf(QRegExp("\\\\" + struct_level2 + "\\*?[\\{\\[]"), tagEnd);
    tagEnd = -1;
    offset = tagStart;
    if ((tagStart != -1) &&
        ((tagComment == -1) ||
  ...
  if (newItem.type >= 0) {
    while (j < templist.count()) {
      if (templist[j].cursor.position() > newItem.cursor.position())
        break;
      ++j;
    }
  ...
  if (newLabelItem.type >= 0) {
    while (j < templist.count()) {
      if (templist[j].cursor.position() > newLabelItem.cursor.position())
        break;
      ++j;
    }
...
LatexEditor::LatexEditor(QWidget *parent, QFont &efont, QList<QColor> edcolors,
                         QList<QColor> hicolors, bool inlinespelling,
                         QString ignoredWords, Hunspell *spellChecker,
                         bool tabspaces, int tabwidth,
                         const QKeySequence viewfocus, QString name,
                         QStringList ulist)
    : QPlainTextEdit(parent), c(0), lastSaveRevision(0) {
  ...
  if (!ignoredWords.isEmpty())
    alwaysignoredwordList = ignoredWords.split(",");
  else
    alwaysignoredwordList.clear();
  ...
  if (wordsfile.open(QFile::ReadOnly)) {
    while (!wordsfile.atEnd()) {
      ...
      if (!line.isEmpty())
        hardignoredwordList.append(line.trimmed());
    }
  }
  /********************************/

  highlighter = new LatexHighlighter(document(), inlinespelling, ignoredWords,
                                     spellChecker);
  highlighter->SetEditor(this);
  highlighter->setColors(hicolors);
  QFileInfo fi(fname);
  if (!fname.startsWith("untitled") &&
      (fi.suffix() == "asy" || fi.suffix() == "mp"))
  ...
  connect(&highlightRemover, SIGNAL(timeout()), this,
          SLOT(clearHightLightLine()));
...
void LatexEditor::paintEvent(QPaintEvent *event) {
  ...
  if (inBlockSelectionMode && context.selections.count() &&
  ...
  while (block.isValid()) {
    ...
    if (r.bottom() >= er.top() && r.top() <= er.bottom()) {
      ...
      for (int i = 0; i < context.selections.size(); ++i) {
        ...
        if (selStart < bllen && selEnd >= 0 && selEnd >= selStart) {
          ...
          if (i == blockSelectionIndex) {
            ...
            o.start = blockSelection.positionAtColumn(
                text, blockSelection.firstVisualColumn, 0);
            o.length = blockSelection.positionAtColumn(
                           text, blockSelection.lastVisualColumn, 0) -
                       o.start;
          }
      ...
      if (inBlockSelectionMode &&
        ...
        int offset = 0;
        int relativePos = blockSelection.positionAtColumn(
            text, blockSelection.firstVisualColumn, &offset);
        QTextLine line = layout->lineForTextPosition(relativePos);
        qreal x = line.cursorToX(relativePos) + offset * spacew;

        int eoffset = 0;
        int erelativePos = blockSelection.positionAtColumn(
            text, blockSelection.lastVisualColumn, &eoffset);
      ...
      if (drawCursorAsBlock) {
        ...
        if (relativePos < line.textLength() - line.textStart()) {
          ...
          w = QFontMetrics(layout->font())
                  .width(QLatin1Char(' ')); // in sync with QTextLine::draw()
  ...
  if (backgroundVisible() && !block.isValid() && offset.y() <= er.bottom() &&
    ...
    painter.fillRect(
        QRect(QPoint((int)er.left(), (int)offset.y()), er.bottomRight()),
        palette().background());
  }
  ...
  if (cursor_layout && !inBlockSelectionMode) {
    ...
    cursor_layout->drawCursor(&painter, cursor_offset, cursor_cpos,
                              cursorWidth());
  }
}
...
void LatexEditor::contextMenuEvent(QContextMenuEvent *e) {
  ...
  if (inlinecheckSpelling && pChecker && !textCursor().hasSelection()) {
    ...
    if (data->misspelled[colstart] == true) {
      while ((colstart >= 0) && (colstart < data->misspelled.count()) &&
             (data->misspelled[colstart] == true)) {
        colstart--;
      }
      colstart++;
      colend = cpos - bpos;
      while ((colend >= 0) && (colend < data->misspelled.count()) &&
             (data->misspelled[colend] == true)) {
        colend++;
      }
      ...
      if (text.length() > 1 && colend > colstart) {
        ...
        if (!ignoredwordList.contains(word) &&
            !hardignoredwordList.contains(word)) {
  ...
  if (!document()->isEmpty() && !inlinecheckSpelling) {
    a = menu->addAction(tr("Check Spelling Word"), this,
                        SLOT(checkSpellingWord()));
    // a->setEnabled(!document()->isEmpty() && !inlinecheckSpelling);
  }
  if (textCursor().hasSelection()) {
    a = menu->addAction(tr("Check Spelling Selection"), this,
                        SLOT(checkSpellingSelection()));
    // a->setEnabled(textCursor().hasSelection());
  }
  a = menu->addAction(tr("Check Spelling Document"), this,
                      SLOT(checkSpellingDocument()));
...
bool LatexEditor::search(const QString &expr, bool cs, bool wo, bool forward,
                         bool startAtCursor, bool isRegExp) {
  ...
  QTextCursor found = isRegExp ? document()->find(QRegExp(expr), c, flags)
                               : document()->find(expr, c, flags);
...
int LatexEditor::searchInSelection(const QString &expr, bool cs, bool wo,
                                   bool isRegExp, int start, int end) {
  QTextDocument::FindFlags flags = 0;
  if (cs)
    flags |= QTextDocument::FindCaseSensitively;
  if (wo)
  ...
  QTextCursor found = isRegExp ? document()->find(QRegExp(expr), start, flags)
                               : document()->find(expr, start, flags);

  if (found.isNull())
    return -1;
  else {
    if (found.selectionEnd() > end)
      return -1;
    else {
      setTextCursor(found);
...
int LatexEditor::searchLine(const QString &expr) {
  ...
  c.setPosition(0);
  // c.movePosition(QTextCursor::Start); //Qt 4.7.1 bug
  QTextCursor found = document()->find(expr, c,
...
void LatexEditor::changeFont(QFont &new_font) { setFont(new_font); }

QString LatexEditor::getEncoding() { return encoding; }

void LatexEditor::setEncoding(QString enc) { encoding = enc; }
...
int LatexEditor::numoflines() { return document()->blockCount(); }
...
QString LatexEditor::commandUnderCursor() const {
  ...
  // 	QRegExp
  // completionStartRegExp("(\\\\([\\s\\{\\}\\[\\]\\w,.=\"'~:]|(\\&)|(\\$)|(\\%)(\\#)(\\_)|(\\{)|(\\})|(\\backslash)|(\\^)|(\\[)|(\\]))*)$");
...
#define IS_WORD_BACK(ch)                                                       \
  (ch.isLetter() || ch.isNumber() || ch.isMark() || ch == '{' || ch == '[' ||  \
   ch == ':' || ch == '_')
#define IS_WORD_FORWARD(ch) (ch.isLetter() || ch.isMark())

  bool isControlSeq =
      false; // becomes true if we include an @ sign or a leading backslash
  ...
  if (IS_WORD_BACK(ch) || ch == '@' /* || ch == '\'' || ch == 0x2019 */) {
    ...
    while (start > 0) {
      ...
      if (!isControlSeq && (ch == '\'' || ch == 0x2019) && start > 0 &&
          IS_WORD_BACK(text.at(start - 1))) {
        includesApos = true;
        continue;
      }
      ++start;
    ...
    while (++end < text.length()) {
      ...
      if (!isControlSeq && (ch == '\'' || ch == 0x2019) &&
          end < text.length() - 1 && IS_WORD_FORWARD(text.at(end + 1))) {
        includesApos = true;
        continue;
      }
      break;
...
QStringList LatexEditor::fullcommandUnderCursor() {
  ...
  int end = index;
  if (index < 0)
    return (QStringList() << "" << QString::number(index)
  ...
  if (index >= text.length())
    return (QStringList() << "" << QString::number(index)
                          << QString::number(start) << QString::number(end));
  QChar ch = text.at(index);
#define IS_WORD_FORMING(ch)                                                    \
  (ch.isLetter() || ch.isNumber() || ch.isMark() || ch == '{' || ch == '[' ||  \
   ch == '}' || ch == ']' || ch == ':' || ch == '_')
  //#define IS_WORD_FORMING(ch) (ch.isLetter() || ch.isMark() || ch=='{' ||
  //ch=='[' || ch=='}' || ch==']')
  bool isControlSeq =
      false; // becomes true if we include an @ sign or a leading backslash
  bool includesApos = false; // becomes true if we include an apostrophe
  ...
  if (IS_WORD_FORMING(ch) || ch == '@' /* || ch == '\'' || ch == 0x2019 */) {
    ...
    while (start > 0) {
      ...
      if (!isControlSeq && (ch == '\'' || ch == 0x2019) && start > 0 &&
          IS_WORD_FORMING(text.at(start - 1))) {
        includesApos = true;
        continue;
      }
      ++start;
    ...
    while (++end < text.length()) {
      ...
      if (!isControlSeq && (ch == '\'' || ch == 0x2019) &&
          end < text.length() - 1 && IS_WORD_FORMING(text.at(end + 1))) {
        includesApos = true;
        continue;
      }
      break;
    ...
    tc.setPosition(block.position() + end, QTextCursor::KeepAnchor);
    result = tc.selectedText();
    return (QStringList() << result << QString::number(index)
                          << QString::number(start) << QString::number(end));
  }
...
void LatexEditor::keyPressEvent(QKeyEvent *e) {
  ...
  if (!ctrlOrShift && (e->key() == Qt::Key_Insert)) {
    ...
    if (tabSpaces)
      insertPlainText(QString().fill(' ', tabWidth));
    else
  ...
  // 	}
    ...
    if (s1.matches(vfocus) == QKeySequence::ExactMatch)
          ...
          if (tagList.count() == 3) {
            ...
            if ((trigger != ":") && (trigger == cursor.selectedText())) {
              ...
              while ((index = rx.indexIn(code, index)) != -1) {
                if (rx.cap(1) == "@")
                  code.replace(rx.pos(1), 1, QString(0x2022));
                else if (rx.cap(1) == "@@")
                  code.replace(rx.pos(1), 2, "@");
      ...
      if (!found)
        QPlainTextEdit::keyPressEvent(e);
    } else if (s1.matches(QKeySequence(Qt::CTRL + Qt::Key_S)) !=
                   QKeySequence::ExactMatch &&
               s1.matches(QKeySequence(Qt::CTRL + Qt::Key_Y)) !=
                   QKeySequence::ExactMatch)
      QPlainTextEdit::keyPressEvent(e);
  }
  if (c && !c->popup()->isVisible()) {
  ...
  if ((c->completionModel()->rowCount() == 1) &&
      (c->completionModel()->data(c->completionModel()->index(0, 0)) ==
       completionPrefix)) {
    c->popup()->hide();
    return;
  }
  if (completionPrefix != c->completionPrefix()) {
    ...
    if (completionPrefix.startsWith("\\beg"))
      c->popup()->setCurrentIndex(
          c->completionModel()->index(c->completionModel()->rowCount() - 1, 0));
    else
  ...
  while (i < c->completionModel()->rowCount()) {
    pos = c->completionModel()
              ->data(c->completionModel()->index(i, 0))
              .toString()
              .indexOf("*");
    if (pos > -1) {
      item = c->completionModel()
                 ->data(c->completionModel()->index(i, 0))
                 .toString()
                 .left(pos);
      i++;
      if (i < c->completionModel()->rowCount()) {
        if (c->completionModel()
                ...
                .startsWith(item)) {
          aa =
              c->completionModel()->data(c->completionModel()->index(i - 1, 0));
          bb = c->completionModel()->data(c->completionModel()->index(i, 0));
          c->completionModel()->setData(c->completionModel()->index(i - 1, 0),
                                        bb);
...
QCompleter *LatexEditor::completer() const { return c; }
...
void LatexEditor::setCompleter(QCompleter *completer) {
  ...
  QObject::connect(c, SIGNAL(activated(const QString &)), this,
...
void LatexEditor::insertCompletion(const QString &completion) {
  ...
  // qDebug() << bef << aft;
  // tc.removeSelectedText();
  if (bef.contains("{") || (!aft.contains("}") && !aft.contains("]")))
    tc.removeSelectedText();
  else
...
bool LatexEditor::isSpace(QChar c) const {
  return c == QLatin1Char(' ') || c == QChar::Nbsp ||
         c == QChar::LineSeparator || c == QLatin1Char('\t');
}
...
void LatexEditor::matchAll() {
  ...
  if (foldableLines.keys().contains(textCursor().block().blockNumber()))
    emit requestGotoStructure(textCursor().block().blockNumber());

  // emit poshaschanged(textCursor().blockNumber() + 1,textCursor().position() -
  // document()->findBlock(textCursor().selectionStart()).position()+1);
  emit poshaschanged(textCursor().blockNumber() + 1,
...
bool LatexEditor::matchLeftPar(QTextBlock currentBlock, int index,
                               int numLeftPar) {
...
bool LatexEditor::matchRightPar(QTextBlock currentBlock, int index,
                                int numRightPar) {
...
void LatexEditor::matchLat() {
  ...
  QTextBlock textBlock = textCursor().block();
  if (foldableLines.keys().contains(textBlock.blockNumber()))
    createLatSelection(textBlock.blockNumber(),
                       foldableLines[textBlock.blockNumber()]);
  else {
    BlockData *data = static_cast<BlockData *>(textBlock.userData());
    if (data) {
      ...
      for (int i = 0; i < infos.size(); ++i) {
        ...
        if (info->position <= curPos && info->character == 'b')
          matchLeftLat(textBlock, i + 1, 0, textBlock.blockNumber());
        if (info->position <= curPos && info->character == 'e')
          matchRightLat(textBlock, i - 1, 0, textBlock.blockNumber());
      }
    }
  }
}

bool LatexEditor::matchLeftLat(QTextBlock currentBlock, int index,
                               int numLeftLat, int bpos) {
...
bool LatexEditor::matchRightLat(QTextBlock currentBlock, int index,
                                int numRightLat, int epos) {
...
void LatexEditor::unfold(int start, int end) {
  if (!foldedLines.keys().contains(start))
    return;
...
{
  disconnect(this, SIGNAL(blockCountChanged(int)), this,
             SLOT(requestNewNumLines(int)));
  QTextCursor cursor = textCursor();
  cursor.movePosition(QTextCursor::End, QTextCursor::MoveAnchor);
  bool emptyFile = !cursor.movePosition(QTextCursor::PreviousCharacter,
                                        QTextCursor::KeepAnchor);
...
int LatexEditor::getLastNumLines() { return lastnumlines; }

void LatexEditor::setLastNumLines(int n) { lastnumlines = n; }
...
QDateTime LatexEditor::getLastSavedTime() { return lastSavedTime; }

void LatexEditor::setLastSavedTime(QDateTime t) { lastSavedTime = t; }
...
void LatexEditor::requestNewNumLines(int n) { emit numLinesChanged(n); }

void LatexEditor::setKeyViewerFocus(QKeySequence s) { vfocus = s; }

void LatexEditor::updateName(QString f) {
  QFileInfo fiold(fname);
  QFileInfo finew(f);
  if ((fname.startsWith("untitled") ||
       (fiold.suffix() != "asy" && fiold.suffix() != "mp")) &&
      (finew.suffix() == "asy" || finew.suffix() == "mp")) {
    highlighter->setModeGraphic(true);
    highlighter->rehighlight();
  }
  fname = f;
...
void LatexEditor::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (e->modifiers() & Qt::AltModifier) {
    if (!inBlockSelectionMode) {
      ...
      QTextCursor cursor = textCursor();
      // get visual column
      int column = blockSelection.columnAt(cursor.block().text(),
                                           cursor.positionInBlock());
      if (cursor.positionInBlock() == cursor.block().length() - 1) {
        column += (e->pos().x() - cursorRect().center().x()) /
                  QFontMetricsF(font()).width(QLatin1Char(' '));
      }
...
void LatexEditor::removeBlockSelection(const QString &text) {
  ...
  for (;;) {
    QString text = block.text();
    int startOffset = 0;
    int startPos = blockSelection.positionAtColumn(
        text, blockSelection.firstVisualColumn, &startOffset);
    int endOffset = 0;
    int endPos = blockSelection.positionAtColumn(
        text, blockSelection.lastVisualColumn, &endOffset);
    ...
    if (startOffset < 0)
      cursor.insertText(
          QString(blockSelection.tabSize + startOffset, QLatin1Char(' ')));
    if (endOffset < 0)
...
QMimeData *LatexEditor::createMimeDataFromSelection() const {
  if (inBlockSelectionMode) {
    ...
    mimeData->setData(QLatin1String("application/vnd.texmaker.vblocktext"),
                      text.toUtf8());
    ...
    // Copy the selected text as HTML
    {
      ...
      for (QTextBlock current = start; current.isValid() && current != end;
        ...
        foreach (const QTextLayout::FormatRange &range,
          ...
          tempCursor.setPosition(qMin(end, endOfDocument),
                                 QTextCursor::KeepAnchor);
    ...
    */
    ...
    if (startOk && multipleBlocks) {
      ...
      mimeData->setData(QLatin1String("application/vnd.texmaker.blocktext"),
                        text.toUtf8());
    }
    return mimeData;
  }
...
void LatexEditor::insertFromMimeData(const QMimeData *source) {
  ...
  if (source->hasFormat(QLatin1String("application/vnd.texmaker.vblocktext"))) {
    QString text = QString::fromUtf8(
        source->data(QLatin1String("application/vnd.texmaker.vblocktext")));
    if (text.isEmpty())
      return;
    ...
    for (int i = 1; i < lines.count(); ++i) {
      ...
      int offset = 0;
      int position = blockSelection.positionAtColumn(cursor.block().text(),
                                                     column, &offset);
...
QString LatexEditor::copyBlockSelection() const {
  ...
  for (;;) {
    ...
    if (startPos == endPos) {
      ...
      if (endOffset < 0) {
        selection +=
            QString(blockSelection.tabSize + endOffset, QLatin1Char(' '));
      } else if (endOffset > 0) {
...
void LatexEditor::setUserTagsList(QStringList utlist) { userTagsList = utlist; }
...
void LatexEditor::dropEvent(QDropEvent *event) {
  ...
  if (event && (uris.size() > 0)) {
    ...
    cursor.setPosition(initialCursorPosition + uri.length(),
                       QTextCursor::KeepAnchor);
    cursor.removeSelectedText();
...
void LatexEditor::insertText(QString text) { textCursor().insertText(text); }

void LatexEditor::setUncommittedLines(QList<int> lines) {
  foreach (int l, lines) {
    if (document()->findBlockByNumber(l - 1).revision() ==
        getlastSaveRevision()) {
