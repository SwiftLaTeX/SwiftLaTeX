...
LineNumberWidget::LineNumberWidget(LatexEditor *editor, QWidget *parent,
                                   bool svn)
    : QWidget(parent), m_editor(editor), m_svn(svn) {
  ...
  connect(m_editor->verticalScrollBar(), SIGNAL(valueChanged(int)), this,
          SLOT(update()));
  connect(m_editor->verticalScrollBar(), SIGNAL(actionTriggered(int)), this,
          SLOT(update()));
  connect(m_editor, SIGNAL(textChanged()), this, SLOT(update()));
  // connect(m_editor->document(), SIGNAL(modificationChanged(bool)), this,
  // SLOT(update()));
  connect(m_editor, SIGNAL(updatelineWidget()), this, SLOT(update()));
  connect(m_editor, SIGNAL(setBlockRange(int, int)), this,
...
void LineNumberWidget::paintEvent(QPaintEvent *event) {
  ...
  while (p.isValid()) {
    ...
    np = p.next();
    if (np.isValid() && np.isVisible())
      delta = (int)m_editor->blockGeometry(np).top() - top;
    else
      delta = fm.lineSpacing();
    if (top < 0) {
      if (i == start + 1) {
        realmin = top;
        if (top >= 0)
      ...
      if (i <= end + 1)
        rmax = top + delta; // top+fm.lineSpacing();
      p = p.next();
      i++;
      continue;
    }
    ...
    for (int j = 0; j < 3; ++j) {
      if (m_editor->UserBookmark[j] == i) {
        ...
        painter.fillRect(2, top, fm.width("0") + 6, fm.lineSpacing(),
                         bookmarkbrush);
        painter.setPen(bookmarkpen);
        painter.drawText(4, top, width() - 4, fm.lineSpacing(),
                         Qt::AlignLeft | Qt::AlignTop, QString::number(j + 1));
      }
    }
    if (m_svn && (m_editor->document()->findBlockByNumber(i - 1).revision() !=
                  m_editor->getlastSaveRevision())) {
      painter.setPen(oldpenSvn);
    } else
      painter.setPen(oldpen);
    numtext = QString::number(i);
    if (p.isVisible())
      painter.drawText(10 + fm.width("0"), top, fm.width(numtext),
                       fm.lineSpacing(), Qt::AlignRight | Qt::AlignTop,
                       numtext);
    ...
    if ((m_editor->foldedLines.keys().contains(i - 1)) && (i != start + 1)) {
      painter.setPen(bookmarkpen);
      painter.fillRect(width() - 20, top + (int)(fm.lineSpacing() / 2) - 5, 10,
                       10, markerbrush);
      painter.drawLine(width() - 17, top + (int)(fm.lineSpacing() / 2),
                       width() - 13, top + (int)(fm.lineSpacing() / 2));
      painter.drawLine(width() - 15, top + (int)(fm.lineSpacing() / 2) - 2,
                       width() - 15, top + (int)(fm.lineSpacing() / 2) + 2);
      rangepen.setStyle(Qt::DotLine);
      painter.setPen(rangepen);
      painter.drawLine(fm.width("0") + 8, top + fm.lineSpacing(), width() - 12,
                       top + fm.lineSpacing());
  ...
  if ((rmin >= 0) && (rmax >= 0)) {
    if (realmin >= 0) {
      painter.setPen(bookmarkpen);
      painter.fillRect(width() - 20, rmin + (int)(fm.lineSpacing() / 2) - 5, 10,
                       10, markerbrush);
      painter.drawLine(width() - 17, rmin + (int)(fm.lineSpacing() / 2),
                       width() - 14, rmin + (int)(fm.lineSpacing() / 2));
      painter.setPen(rangepen);
      if (m_editor->foldedLines.keys().contains(start)) {
        painter.setPen(bookmarkpen);
        painter.drawLine(width() - 17, rmin + (int)(fm.lineSpacing() / 2),
                         width() - 13, rmin + (int)(fm.lineSpacing() / 2));
        painter.drawLine(width() - 15, rmin + (int)(fm.lineSpacing() / 2) - 2,
                         width() - 15, rmin + (int)(fm.lineSpacing() / 2) + 2);
        rangepen.setStyle(Qt::DotLine);
        painter.setPen(rangepen);
        painter.drawLine(fm.width("0") + 8, rmin + fm.lineSpacing(),
                         width() - 12, rmin + fm.lineSpacing());
        rangepen.setStyle(Qt::SolidLine);
        painter.setPen(rangepen);
      } else {
        // painter.drawLine(width()-7,rmin+2,width()-7,qMin(rmax,height()));
        painter.fillRect(width() - 7, rmin + 2, 2,
                         qMin(rmax, height()) - rmin - 2, markerbrush);
        // painter.drawLine(width()-6,rmin+2,width()-6,qMin(rmax,height()));
      }

    } else {
      // painter.drawLine(width()-7,rmin,width()-7,qMin(rmax,height()));
      painter.fillRect(width() - 7, rmin, 2, qMin(rmax, height()) - rmin,
                       markerbrush);
...
void LineNumberWidget::setFont(QFont efont) { numfont = efont; }

void LineNumberWidget::setSvn(bool svn) { m_svn = svn; }
  ...
  if (event->type() == QEvent::ToolTip) {
    ...
    if (abs(p) >= fm.width("0") + 8) {
      ...
      QToolTip::showText(helpEvent->globalPos(),
                         tr("Click to add or remove a bookmark"));
    return true;
  }
