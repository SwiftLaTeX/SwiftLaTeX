...
SpellerDialog::SpellerDialog(QWidget *parent, LatexEditor *ed,
                             QString ignoredWords)
    : QDialog(parent) {
  editor = ed;
  ...
  connect(ui.pushButtonAlwaysIgnore, SIGNAL(clicked()), this,
          SLOT(slotAlwaysIgnore()));
  ...
  // else ui.labelMessage->setText("<b>"+tr("Error : Can't open the
  // dictionary")+"</b>");
}
...
void SpellerDialog::SpellingNextWord() {
  ...
  //	c.movePosition(QTextCursor::NextCharacter,QTextCursor::MoveAnchor);
          ...
          cols=colstart;
          text=c.selectedText();
          while ((cols<colend && cols<data->code.count() && data->code[cols]==1)
  || text.mid(cols-colstart,1)==" " || text.mid(cols-colstart,1)=="\t" )
                  {
                  cols++;
                  }
          cole=colend;
          while (cole>colstart && cole<data->code.count() &&
  data->code[cole]==1)
                  {
                  cole--;
                  }
  ...
  !hardignoredwordList.contains(word))
                          ...
                          if (pChecker) check =
  pChecker->spell(encodedString.data()); else check=true; if (!check)
                                  {
                                  ...
                                  gonext=false;
                                  ns =
  pChecker->suggest(&wlst,encodedString.data()); if (ns > 0)
                                          {
                                          suggWords.clear();
                                          for (int i=0; i < ns; i++)
                                                  {
  ...
  while (gonext && c.position() < endpos + deltacol && block.isValid()) {
    ...
    while (gonext && i < text.length()) {
      ...
      if ((i - buffer.length() - offset + block.position() <=
           endpos + deltacol) &&
          (i - offset + block.position() >= c.position()) &&
          (buffer.length() > 1) && (!ignoredwordList.contains(buffer)) &&
        ...
        if (pChecker)
          ...
          check = true;
        if (!check) {
          editor->selectword(block.blockNumber(), i - buffer.length() - offset,
                             buffer);
          ui.listWidget->setEnabled(true);
          ui.lineEditNew->setEnabled(true);
          ui.pushButtonIgnore->setEnabled(true);
