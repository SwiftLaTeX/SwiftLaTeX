...
QPdfMutexLocker::QPdfMutexLocker() : QMutexLocker(pdfMutex()) {}

QPdfDocumentPrivate::QPdfDocumentPrivate()
    : avail(nullptr), doc(nullptr), loadComplete(false),
      status(QPdfDocument::Null), lastError(QPdfDocument::NoError),
      pageCount(0) {
...
void QPdfDocumentPrivate::updateLastError() {
  ...
  case FPDF_ERR_UNKNOWN:
    lastError = QPdfDocument::UnknownError;
    break;
  case FPDF_ERR_FILE:
    lastError = QPdfDocument::FileNotFoundError;
    break;
  case FPDF_ERR_FORMAT:
    lastError = QPdfDocument::InvalidFileFormatError;
    break;
  case FPDF_ERR_PASSWORD:
    lastError = QPdfDocument::IncorrectPasswordError;
    break;
  case FPDF_ERR_SECURITY:
    lastError = QPdfDocument::UnsupportedSecuritySchemeError;
    break;
  default:
    Q_UNREACHABLE();
  }
...
void QPdfDocumentPrivate::load(QIODevice *newDevice,
  ...
  if (newDevice->isSequential()) {
    ...
    device = &asyncBuffer;
    QNetworkReply *reply =
        qobject_cast<QNetworkReply *>(sequentialSourceDevice);

    if (!reply) {
      setStatus(QPdfDocument::Error);
      qWarning() << "QPdfDocument: Loading from sequential devices only "
                    "supported with QNetworkAccessManager.";
      return;
    }
    ...
    if (reply->header(QNetworkRequest::ContentLengthHeader).isValid())
      ...
      QObject::connect(reply, SIGNAL(metaDataChanged()), q,
                       SLOT(_q_tryLoadingWithSizeFromContentHeader()));
  } else {
    device = newDevice;
...
void QPdfDocumentPrivate::_q_tryLoadingWithSizeFromContentHeader() {
  ...
  QObject::connect(sequentialSourceDevice, SIGNAL(readyRead()), q,
                   SLOT(_q_copyFromSequentialSourceDevice()));
...
void QPdfDocumentPrivate::initiateAsyncLoadWithTotalSizeKnown(
    quint64 totalSize) {
  // FPDF_FILEACCESS setup
  m_FileLen = totalSize;
...
void QPdfDocumentPrivate::_q_copyFromSequentialSourceDevice() {
  ...
  const QByteArray data =
      sequentialSourceDevice->read(sequentialSourceDevice->bytesAvailable());
  if (data.isEmpty())
    return;
...
FPDF_BOOL QPdfDocumentPrivate::fpdf_IsDataAvail(_FX_FILEAVAIL *pThis,
...
int QPdfDocumentPrivate::fpdf_GetBlock(void *param, unsigned long position,
                                       unsigned char *pBuf,
                                       unsigned long size) {
  QPdfDocumentPrivate *d = static_cast<QPdfDocumentPrivate *>(
      reinterpret_cast<FPDF_FILEACCESS *>(param));
  d->device->seek(position);
  return qMax(qint64(0), d->device->read(reinterpret_cast<char *>(pBuf), size));
}

void QPdfDocumentPrivate::fpdf_AddSegment(_FX_DOWNLOADHINTS *pThis,
                                          size_t offset, size_t size) {
  Q_UNUSED(pThis);
  Q_UNUSED(offset);
  Q_UNUSED(size);
...
QPdfDocument::QPdfDocument(QObject *parent)
    : QObject(parent), d(new QPdfDocumentPrivate) {
  d->q = this;
}
...
/*!
    ...
    \value Null The initial status after the document has been created or after
   it has been closed. \value Loading The status after load() has been called
   and before the document is fully loaded. \value Ready The status when the
   document is fully loaded and its data can be accessed. \value Unloading The
   status after close() has been called on an open document. At this point the
   document is still valid and all its data can be accessed. \value Error The
   status after Loading, if loading has failed.
...
*/
QPdfDocument::Status QPdfDocument::status() const { return d->status; }
...
/*!
   ...
   document from which it was converted as QString. \value Producer If the
   document was converted to PDF from another format, the name of the conforming
   product that converted it to PDF as QString. \value CreationDate The date and
   time the document was created as QDateTime. \value ModificationDate The date
   and time the document was most recently modified as QDateTime.
...
QVariant QPdfDocument::metaData(MetaDataField field) const {
  ...
  const unsigned long len =
      FPDF_GetMetaText(d->doc, fieldName.constData(), nullptr, 0);
...
QPdfDocument::DocumentError QPdfDocument::error() const { return d->lastError; }
...
int QPdfDocument::pageCount() const { return d->pageCount; }
...
*/
QImage QPdfDocument::render(int page, QSize imageSize,
  ...
  FPDF_BITMAP bitmap =
      FPDFBitmap_CreateEx(result.width(), result.height(), FPDFBitmap_BGRA,
                          result.bits(), result.bytesPerLine());
  ...
  FPDF_RenderPageBitmap(bitmap, pdfPage, 0, 0, result.width(), result.height(),
                        rotation, flags);
...
QString QPdfDocument::checkTextAt(int index, QRectF boundingRect,
  ...
  double page_x;
  double page_y;
  FPDF_DeviceToPage(page, 0, 0, boundingRect.width(), boundingRect.height(), 0,
                    selectionRect.left(), selectionRect.top(), &page_left,
                    &page_top);
  FPDF_DeviceToPage(page, 0, 0, boundingRect.width(), boundingRect.height(), 0,
                    selectionRect.right(), selectionRect.bottom(), &page_right,
                    &page_bottom);
  FPDF_TEXTPAGE textPage = FPDFText_LoadPage(page);
  int txtSize = FPDFText_GetBoundedText(textPage, page_left, page_top,
                                        page_right, page_bottom, NULL, 0);
  unsigned short *txtBuf = new unsigned short[txtSize];
  FPDFText_GetBoundedText(textPage, page_left, page_top, page_right,
                          page_bottom, txtBuf, txtSize);
  result = QString::fromUtf16(txtBuf);
  FPDF_ClosePage(page);
  return result;
}

struct linkItem QPdfDocument::checkLinkAt(int index, QRectF boundingRect, int x,
                                          int y) {
...
struct findItem QPdfDocument::find(int index, QString text, int lastpos) {
  ...
  while (FPDFText_FindNext(find)) {
    ...
    if (charIndex > lastpos) {
      ...
      FPDF_PageToDevice(page, 0, 0, pageSize(index).width(),
                        pageSize(index).height(), 0, left, top, &screenLeft,
                        &screenTop);
      FPDF_PageToDevice(page, 0, 0, pageSize(index).width(),
                        pageSize(index).height(), 0, right, bottom,
