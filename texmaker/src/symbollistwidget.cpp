...
SymbolListWidget ::SymbolListWidget(QWidget *parent, int page)
    : QTableWidget(parent) {
  hide();
  ...
  connect(this, SIGNAL(customContextMenuRequested(const QPoint &)), this,
          SLOT(customContentsMenu(const QPoint &)));
  ...
  case 4: {
    ...
    for (uint i = 0; i <= 39; ++i) {
      icon_name =
          ":/symbols/inverted_img" + QString::number(i + 1) + "greek.png";
      QTableWidgetItem *item = new QTableWidgetItem();
      item->setIcon(QIcon(icon_name));
...
void SymbolListWidget::SetUserPage(usercodelist ulist) {
  ...
  for (uint i = 0; i <= 11; ++i) {
    if ((ulist[i] > -1) && (ulist[i] < 412)) {
      QTableWidgetItem *item = new QTableWidgetItem();
      if (ulist[i] >= 372) {
        icon_name = ":/symbols/inverted_img" + QString::number(ulist[i] - 371) +
                    "greek.png";
      } else {
        icon_name =
            ":/symbols/inverted_img" + QString::number(ulist[i] + 1) + ".png";
      }
...
void SymbolListWidget::SetFavoritePage(QList<int> flist) {
  ...
  for (int i = 0; i < flist.count(); i++) {
    if ((flist.at(i) > -1) && (flist.at(i) < 412)) {
      QTableWidgetItem *item = new QTableWidgetItem();
      if (flist.at(i) >= 372) {
        icon_name = ":/symbols/inverted_img" +
                    QString::number(flist.at(i) - 371) + "greek.png";
      } else {
        icon_name = ":/symbols/inverted_img" +
                    QString::number(flist.at(i) + 1) + ".png";
      }
