...
LatexEditorView::LatexEditorView(QWidget *parent, QFont &efont, bool svn,
                                 bool line, QList<QColor> edcolors,
  ...
  editor = new LatexEditor(frame, efont, edcolors, hicolors, inlinespelling,
                           ignoredWords, spellChecker, tabspaces, tabwidth,
                           viewfocus, name, ulist);
  ...
  connect(replacewidget, SIGNAL(requestExtension()), this,
          SLOT(updateReplace()));
...
void LatexEditorView::showFind() {
  ...
  if (c.hasSelection())
    findwidget->ui.comboFind->lineEdit()->setText(c.selectedText());
...
void LatexEditorView::showFindNext() {
  ...
  Stack->setMaximumHeight(findwidget->minimumSizeHint().height());
  Stack->show();
  if (findwidget->ui.comboFind->lineEdit()->text() != "")
    findwidget->doFind();
}
...
void LatexEditorView::showReplace() {
  ...
  QTextCursor c = editor->textCursor();
  if (c.hasSelection())
    replacewidget->ui.comboFind->lineEdit()->setText(c.selectedText());
