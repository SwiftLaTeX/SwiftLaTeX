...
ReplaceWidget::ReplaceWidget(QWidget *parent) : QWidget(parent) {
  ...
  connect(ui.checkSelection, SIGNAL(toggled(bool)), this,
          SLOT(updateSelection(bool)));
...
void ReplaceWidget::doReplaceAll() {
  ...
  if (ui.checkSelection->isChecked()) {
    ...
    while (editor->search(
        ...
        !ui.checkBegin->isChecked(), ui.checkRegExp->isChecked())) {
      editor->replace(ui.comboReplace->currentText(),
                      ui.checkRegExp->isChecked(), ui.comboFind->currentText());
      ui.checkBegin->setChecked(false);
    }
    ui.checkBegin->setChecked(true);
  }
...
void ReplaceWidget::SetEditor(LatexEditor *ed) { editor = ed; }
...
void ReplaceWidget::updateSelection(bool e) {
  ...
  if (e) {
    ...
    disconnect(ui.checkRegExp, SIGNAL(toggled(bool)), this,
               SLOT(updateReg(bool)));
    ui.checkRegExp->setChecked(false);
    ui.checkRegExp->setEnabled(false);
    connect(ui.checkRegExp, SIGNAL(toggled(bool)), this, SLOT(updateReg(bool)));
  } else
    ui.checkRegExp->setEnabled(true);
  if (!editor) {
    startpos = -1;
    endpos = -1;
    deltacol = 0;
...
void ReplaceWidget::updateReg(bool e) {
  if (e) {
    disconnect(ui.checkSelection, SIGNAL(toggled(bool)), this,
               SLOT(updateSelection(bool)));
    ui.checkSelection->setChecked(false);
    ...
    ui.checkSelection->setEnabled(true);
}
...
void ReplaceWidget::replaceSelection() {
  ...
  if (!newword.isEmpty()) {
    ...
    editor->replace(ui.comboReplace->currentText(), ui.checkRegExp->isChecked(),
                    ui.comboFind->currentText());
