...
public:
  ...
  void moveAnchor(int blockNumber, int visualColumn) {
    ...
    if (blockNumber >= 0 && blockNumber < firstBlock.document()->blockCount()) {
      if (anchor <= TopRight) {
        firstBlock.setPosition(
            firstBlock.document()->findBlockByNumber(blockNumber).position());
        if (firstBlock.blockNumber() > lastBlock.blockNumber()) {
          qSwap(firstBlock, lastBlock);
          anchor = (Anchor)(anchor + 2);
        }
      } else {
        lastBlock.setPosition(
            firstBlock.document()->findBlockByNumber(blockNumber).position());
  ...
  inline int anchorBlockNumber() const {
    return (anchor <= TopRight ? firstBlock.blockNumber()
  ...
  QTextCursor selection() const {
    ...
    if (anchor <= TopRight) {
      cursor.setPosition(
          lastBlock.block().position() +
          positionAtColumn(lastBlock.block().text(), lastVisualColumn, 0));
      cursor.setPosition(
          firstBlock.block().position() +
              positionAtColumn(firstBlock.block().text(), firstVisualColumn, 0),
  ...
  void fromSelection(const QTextCursor &selection) {
    ...
    firstVisualColumn =
        columnAt(firstBlock.block().text(), firstBlock.positionInBlock());
    lastBlock = selection;
    lastBlock.setPosition(selection.selectionEnd());
    lastVisualColumn =
        columnAt(lastBlock.block().text(), lastBlock.positionInBlock());
