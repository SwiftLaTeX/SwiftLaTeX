...
int PageItem::cacheSize() { return s_cache.totalCost(); }

void PageItem::setCacheSize(int cacheSize) { s_cache.setMaxCost(cacheSize); }

bool PageItem::decoratePages() { return s_decoratePages; }

void PageItem::setDecoratePages(bool decoratePages) {
  s_decoratePages = decoratePages;
}

bool PageItem::decorateLinks() { return s_decorateLinks; }

void PageItem::setDecorateLinks(bool decorateLinks) {
  s_decorateLinks = decorateLinks;
}

bool PageItem::invertColors() { return s_invertColors; }

void PageItem::setInvertColors(bool invertColors) {
  s_invertColors = invertColors;
}

PageItem::PageItem(QMutex *mutex, QPdfDocument *doc, int index,
                   QGraphicsItem *parent)
    : QGraphicsObject(parent), m_mutex(0), m_doc(0), m_index(-1), m_size(),
      m_highlights(), m_rubberBand(), m_physicalDpiX(72), m_physicalDpiY(72),
      m_scaleFactor(1.0), m_rotation(QPdf::Rotation::Rotate0), m_transform(),
      m_normalizedTransform(), m_boundingRect(), m_image(), m_render(0) {
  ...
  connect(this, SIGNAL(imageReady(QImage, bool)),
          SLOT(on_imageReady(QImage, bool)));
...
void PageItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *,
                     QWidget *) {
  QImage image;
  ...
  if (s_decoratePages) {
    painter->fillRect(m_boundingRect,
                      QBrush(s_invertColors ? Qt::black : Qt::white));
  ...
  if (!highlightPath.isEmpty()) {
    QRect highlightRect =
        matrix().mapRect(highlightPath.boundingRect()).toRect();
    highlightRect.adjust(-3, -2, 3, 0);
    //      QImage highlight = image.copy(highlightRect);
    //      painter.begin(&image);
    painter->fillRect(highlightRect, blCol);
...
int PageItem::index() const { return m_index; }

QSizeF PageItem::size() const { return m_size; }

const QList<QRectF> &PageItem::highlights() const { return m_highlights; }
...
int PageItem::physicalDpiX() const { return m_physicalDpiX; }

int PageItem::physicalDpiY() const { return m_physicalDpiY; }
...
qreal PageItem::scaleFactor() const { return m_scaleFactor; }
...
QPdf::Rotation PageItem::rotation() const { return m_rotation; }
...
const QTransform &PageItem::transform() const { return m_transform; }
...
void PageItem::startRender(bool prefetch) {
  if (!m_render->isRunning()) {
    m_render->setFuture(QtConcurrent::run(this, &PageItem::render,
                                          m_physicalDpiX, m_physicalDpiY,
                                          m_scaleFactor, m_rotation, prefetch));
  }
}
...
void PageItem::on_render_finished() { update(); }
...
void PageItem::hoverMoveEvent(QGraphicsSceneHoverEvent *event) {
  ...
  if (event->modifiers() == Qt::NoModifier) {
    ...
    if (m_doc
            ->checkLinkAt(m_index, boundingRect(), event->pos().x(),
                          event->pos().y())
            .foundLink)
...
void PageItem::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if (event->modifiers() == Qt::NoModifier &&
    ...
    struct linkItem link = m_doc->checkLinkAt(
        m_index, boundingRect(), event->pos().x(), event->pos().y());
    if (link.foundLink) {
      if (link.actionLink == 1)
        emit linkClicked(link.destPageLink + 1, 0.0, 0.0);
      if (link.actionLink == 2)
        emit linkClicked(link.uriLink);
    }

  } else if (
      (event->modifiers() ==
       Qt::ShiftModifier /*|| event->modifiers() == Qt::ControlModifier*/) &&
      event->button() == Qt::LeftButton) {
...
void PageItem::copyToClipboard(const QPoint &screenPos) {
  ...
  if (action == copyTextAction) {
    ...
    //
    //         text =
    //         m_page->text(m_transform.inverted().mapRect(m_rubberBand));
    //
    //         m_mutex->unlock();
    //
    if (!text.isEmpty()) {
    ...
    image = m_doc->render(m_index,
                          QSize(size.width() * m_physicalDpiX / 72,
                                size.height() * m_physicalDpiY / 72),
                          options);
    //             break;
    //         case QPdf::Rotation::Rotate90:
    //             image = m_doc->render(m_index,QSize(size.height()*
    //             m_physicalDpiY/72,size.width()* m_physicalDpiX/72),options);
    //             //image = m_page->renderToImage(scaleFactor * physicalDpiX,
    //             scaleFactor * physicalDpiY, -1, -1, -1, -1, rotation); break;
    //         case QPdf::Rotation::Rotate180:
    //         image = m_doc->render(m_index,QSize(size.width()*
    //         m_physicalDpiX/72,size.height()* m_physicalDpiY/72),options);
    //         break;
    //         case QPdf::Rotation::Rotate270:
    //             image = m_doc->render(m_index,QSize(size.height()*
    //             m_physicalDpiY/72,size.width()* m_physicalDpiX/72),options);
    //             //image = m_page->renderToImage(scaleFactor * physicalDpiY,
    //             scaleFactor * physicalDpiX, -1, -1, -1, -1, rotation);
    //         break;
    //     }
...
void PageItem::prepareGeometry() {
  ...
  if (qApp->devicePixelRatio() >= 2)
    extra = qApp->devicePixelRatio();

  switch (m_rotation) {
  case QPdf::Rotation::Rotate0:
  case QPdf::Rotation::Rotate90:
    m_transform.scale(m_scaleFactor * m_physicalDpiX / 72.0 / extra,
                      m_scaleFactor * m_physicalDpiY / 72.0 / extra);
    m_normalizedTransform.scale(
        m_scaleFactor * m_physicalDpiX / 72.0 / extra * m_size.width(),
        m_scaleFactor * m_physicalDpiY / 72.0 / extra * m_size.height());
    break;
  case QPdf::Rotation::Rotate180:
  case QPdf::Rotation::Rotate270:
    m_transform.scale(m_scaleFactor * m_physicalDpiY / 72.0 / extra,
                      m_scaleFactor * m_physicalDpiX / 72.0 / extra);
    m_normalizedTransform.scale(
        m_scaleFactor * m_physicalDpiY / 72.0 / extra * m_size.width(),
        m_scaleFactor * m_physicalDpiX / 72.0 / extra * m_size.height());
    break;
  }
...
void PageItem::render(int physicalDpiX, int physicalDpiY, qreal scaleFactor,
  ...
  //    image = m_doc->render(m_index,QSize(size.width()*
  //    physicalDpiX/72,size.height()* physicalDpiY/72),options);
  switch (rotation) {
  case QPdf::Rotation::Rotate0:
    image = m_doc->render(m_index,
                          QSize(size.width() * physicalDpiX / 72,
                                size.height() * physicalDpiY / 72),
                          options);
    break;
  case QPdf::Rotation::Rotate90:
    image = m_doc->render(m_index,
                          QSize(size.height() * physicalDpiY / 72,
                                size.width() * physicalDpiX / 72),
                          options);
    // image = m_page->renderToImage(scaleFactor * physicalDpiX, scaleFactor *
    // physicalDpiY, -1, -1, -1, -1, rotation);
    break;
  case QPdf::Rotation::Rotate180:
    image = m_doc->render(m_index,
                          QSize(size.width() * physicalDpiX / 72,
  ...
  case QPdf::Rotation::Rotate270:
    image = m_doc->render(m_index,
                          QSize(size.height() * physicalDpiY / 72,
                                size.width() * physicalDpiX / 72),
                          options);
    // image = m_page->renderToImage(scaleFactor * physicalDpiY, scaleFactor *
    // physicalDpiX, -1, -1, -1, -1, rotation);
    break;
  }
...
void PageItem::requestNumWords() { emit wantNumWords(); }

void PageItem::requestPageWords() { emit wantNumPageWords(m_index); }

void PageItem::requestPngExport() { emit wantPngExport(m_index); }

void PageItem::requestOpenLocation() { emit wantOpenLocation(); }
