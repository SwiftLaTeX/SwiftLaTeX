...
bool DocumentView::openUrl() { return s_openUrl; }

void DocumentView::setOpenUrl(bool openUrl) { s_openUrl = openUrl; }

bool DocumentView::autoRefresh() { return s_autoRefresh; }

void DocumentView::setAutoRefresh(bool autoRefresh) {
  s_autoRefresh = autoRefresh;
}

bool DocumentView::antialiasing() { return s_antialiasing; }

void DocumentView::setAntialiasing(bool antialiasing) {
  s_antialiasing = antialiasing;
}

bool DocumentView::textAntialiasing() { return s_textAntialiasing; }

void DocumentView::setTextAntialiasing(bool textAntialiasing) {
  s_textAntialiasing = textAntialiasing;
}

bool DocumentView::textHinting() { return s_textHinting; }

void DocumentView::setTextHinting(bool textHinting) {
  s_textHinting = textHinting;
}

bool DocumentView::prefetch() { return s_prefetch; }

void DocumentView::setPrefetch(bool prefetch) { s_prefetch = prefetch; }

qreal DocumentView::pageSpacing() { return s_pageSpacing; }
...
qreal DocumentView::minimumScaleFactor() { return s_minimumScaleFactor; }

qreal DocumentView::maximumScaleFactor() { return s_maximumScaleFactor; }

DocumentView::DocumentView(QWidget *parent)
    : QGraphicsView(parent), m_autoRefreshWatcher(0), m_autoRefreshTimer(0),
      m_prefetchTimer(0), m_mutex(), m_document(0), m_filePath(),
      m_numberOfPages(-1), m_currentPage(-1), m_returnToPage(-1),
      m_returnToLeft(0.0), m_returnToTop(0.0), m_continuousMode(false),
      m_twoPagesMode(false), m_scaleMode(ScaleFactor), m_scaleFactor(0.1),
      m_rotation(QPdf::Rotate0), m_pagesScene(0), m_pages(), m_heightToIndex(),
      m_highlight(0), m_outlineModel(0), m_propertiesModel(0) {
  ...
  connect(verticalScrollBar(), SIGNAL(valueChanged(int)), this,
          SLOT(on_verticalScrollBar_valueChanged(int)));
  connect(horizontalScrollBar(), SIGNAL(valueChanged(int)), this,
          SLOT(on_horizontalScrollBar_valueChanged(int)));
  ...
  connect(m_autoRefreshWatcher, SIGNAL(fileChanged(QString)),
          m_autoRefreshTimer, SLOT(start()));
  ...
  connect(this, SIGNAL(currentPageChanged(int)), m_prefetchTimer,
          SLOT(start()));
  connect(this, SIGNAL(scaleModeChanged(DocumentView::ScaleMode)),
          m_prefetchTimer, SLOT(start()));
  connect(this, SIGNAL(scaleFactorChanged(qreal)), m_prefetchTimer,
          SLOT(start()));
  connect(this, SIGNAL(rotationChanged(QPdf::Rotation)), m_prefetchTimer,
          SLOT(start()));

  connect(m_prefetchTimer, SIGNAL(timeout()), this,
          SLOT(on_prefetch_timeout()));
}

DocumentView::~DocumentView() { qDeleteAll(m_pages); }
...
void DocumentView::clearAll() {
  ...
  disconnect(horizontalScrollBar(), SIGNAL(valueChanged(int)), this,
             SLOT(on_horizontalScrollBar_valueChanged(int)));
  disconnect(m_autoRefreshWatcher, SIGNAL(fileChanged(QString)),
             m_autoRefreshTimer, SLOT(start()));
  disconnect(m_autoRefreshTimer, SIGNAL(timeout()), this, SLOT(refresh()));
  disconnect(this, SIGNAL(currentPageChanged(int)), m_prefetchTimer,
             SLOT(start()));
  disconnect(this, SIGNAL(scaleModeChanged(DocumentView::ScaleMode)),
             m_prefetchTimer, SLOT(start()));
  disconnect(this, SIGNAL(scaleFactorChanged(qreal)), m_prefetchTimer,
             SLOT(start()));
  disconnect(this, SIGNAL(rotationChanged(QPdf::Rotation)), m_prefetchTimer,
             SLOT(start()));
  disconnect(m_prefetchTimer, SIGNAL(timeout()), this,
             SLOT(on_prefetch_timeout()));
  m_pages.clear();
  m_numberOfPages = -1;
  scene()->clear();
...
const QString &DocumentView::filePath() const { return m_filePath; }

int DocumentView::numberOfPages() const { return m_numberOfPages; }

int DocumentView::currentPage() const { return m_currentPage; }

bool DocumentView::continousMode() const { return m_continuousMode; }
...
bool DocumentView::twoPagesMode() const { return m_twoPagesMode; }
...
void DocumentView::setTwoPagesMode(bool twoPagesMode) {
  ...
  if (m_twoPagesMode != twoPagesMode) {
    ...
    if (m_twoPagesMode) {
      if (m_currentPage !=
          (m_currentPage % 2 != 0 ? m_currentPage : m_currentPage - 1)) {
        m_currentPage =
            m_currentPage % 2 != 0 ? m_currentPage : m_currentPage - 1;
...
DocumentView::ScaleMode DocumentView::scaleMode() const { return m_scaleMode; }
...
qreal DocumentView::scaleFactor() const { return m_scaleFactor; }
...
void DocumentView::setScaleFactor(qreal scaleFactor) {
  if (m_pages.count() < 1)
    return;
  if (m_scaleFactor != scaleFactor && scaleFactor >= s_minimumScaleFactor &&
      scaleFactor <= s_maximumScaleFactor) {
...
QPdfDocument *DocumentView::doc() const { return m_document; }

QPdf::Rotation DocumentView::rotation() const { return m_rotation; }
...
void DocumentView::clearPaths(int index) {
  if ((m_pages.count() < 1) || (index >= m_pages.count()))
    return;
...
QPdfBookmarkModel *DocumentView::outlineModel() const { return m_outlineModel; }
...
bool DocumentView::print(QPrinter *printer) {
  ...
  for (int index = fromPage - 1; index <= toPage - 1; index++) {
...

    {
      ...
      ////????????????????????????
      QImage image = m_document->render(
          index, QSize(size.width() * printer->physicalDpiX() / 72,
                       size.height() * printer->physicalDpiY() / 72));

      // QImage image = page->renderToImage(printer->physicalDpiX(),
      // printer->physicalDpiY());

      m_mutex.unlock();

      qreal scaleFactor =
          qMin(printer->width() / pageWidth, printer->height() / pageHeight);
...
void DocumentView::jumpToPage(int page, qreal changeLeft, qreal changeTop,
  ...
  if (page >= 1 && page <= m_numberOfPages) {
    ...
    if (m_currentPage != currentPageForPage(page) ||
        !qFuzzyCompare(1.0 + left, 1.0 + changeLeft) ||
        !qFuzzyCompare(1.0 + top, 1.0 + changeTop)) {
      if (returnTo) {
...
void DocumentView::presentation() {
  ...
  PresentationView *presentationView =
      new PresentationView(&m_mutex, m_document);
...
void DocumentView::on_verticalScrollBar_valueChanged(int value) {
  ...
  if (m_continuousMode) {
    ...
    QMap<qreal, int>::const_iterator lowerBound =
        m_heightToIndex.lowerBound(-value);
...
void DocumentView::keyPressEvent(QKeyEvent *event) {
    ...
    if (!m_continuousMode) {
      if (event->key() == Qt::Key_PageUp &&
        ...
        return;
                 ...
                 m_currentPage != currentPageForPage(m_numberOfPages)) {
        nextPage();
  ...
  if (hasFocus() && (s1.matches(QKeySequence(Qt::CTRL + Qt::Key_Left)) ==
                     QKeySequence::ExactMatch))
    rotateLeft();
  else if (hasFocus() && (s1.matches(QKeySequence(Qt::CTRL + Qt::Key_Right)) ==
                          QKeySequence::ExactMatch))
    rotateRight();
  else
...
void DocumentView::mousePressEvent(QMouseEvent *event) {
  ...
  //     if(!event->isAccepted() && event->modifiers() == Qt::NoModifier &&
  //     event->button() == Qt::RightButton)
  //     {
  //         QMenu* menu = new QMenu();
  //
  //         QAction* returnToPageAction = menu->addAction(tr("&Return to page
  //         %1").arg(m_returnToPage));
  //         returnToPageAction->setShortcut(QKeySequence(Qt::Key_Return));
  //         returnToPageAction->setIcon(QIcon::fromTheme("go-jump",
  //         QIcon(":icons/go-jump.svg")));
  //         returnToPageAction->setIconVisibleInMenu(true);
  //
  //         returnToPageAction->setVisible(m_returnToPage != -1);
  //
  //         QAction* previousPageAction = menu->addAction(tr("&Previous
  //         page"));
  //         previousPageAction->setShortcut(QKeySequence(Qt::Key_Backspace));
  //         previousPageAction->setIcon(QIcon::fromTheme("go-previous",
  //         QIcon(":icons/go-previous.svg")));
  ...
  //         nextPageAction->setIcon(QIcon::fromTheme("go-next",
  //         QIcon(":icons/go-next.svg")));
...
void DocumentView::preparePages() {
  ...
  for (int index = 0; index < m_numberOfPages; index++) {
    ...
    if (qApp->devicePixelRatio() >= 2)
      page->setPhysicalDpi(physicalDpiX() * dpr, physicalDpiY() * dpr);
    else
    ...
    connect(page, SIGNAL(linkClicked(int, qreal, qreal)),
            SLOT(on_pages_linkClicked(int, qreal, qreal)));
    connect(page, SIGNAL(linkClicked(QString)),
            SLOT(on_pages_linkClicked(QString)));
    connect(page, SIGNAL(syncpage(int, const QPointF &)), this,
            SLOT(on_syncpage(int, const QPointF &)));
    connect(page, SIGNAL(wantPngExport(int)), this, SLOT(pngExport(int)));
    connect(page, SIGNAL(wantNumWords()), this, SLOT(countWords()));
    connect(page, SIGNAL(wantNumPageWords(int)), this,
            SLOT(countPageWords(int)));
  ...
  if (PageItem::decoratePages()) {
    ...
    m_pagesScene->setBackgroundBrush(
        QBrush(PageItem::invertColors() ? Qt::black : Qt::white));
  }
}
...
void DocumentView::prepareScene() {
  ...
  for (int index = 0; index < m_numberOfPages; index++) {
    ...
    if (m_scaleMode != ScaleFactor) {
      ...
      case FitToPageSize:
        scaleFactor =
            qMin(visibleWidth / pageWidth, visibleHeight / pageHeight);
        break;
      }
  ...
  for (int index = 0; index < m_numberOfPages; index++) {
    ...
    if (m_twoPagesMode) {
      if (index % 2 == 0) {
        page->setPos(-boundingRect.left() - boundingRect.width() -
                         0.5 * s_pageSpacing,
                     height - boundingRect.top());

        m_heightToIndex.insert(-height + s_pageSpacing + 0.3 * pageHeight,
                               index);
        ...
        page->setPos(-boundingRect.left() + 0.5 * s_pageSpacing,
                     height - boundingRect.top());
      ...
      page->setPos(-boundingRect.left() - 0.5 * boundingRect.width(),
                   height - boundingRect.top());
  ...
  for (int index = 0; index < m_pages.count(); index++) {
    ...
    if (m_continuousMode) {
      ...
      if (index == m_currentPage - 1) {
        ...
        horizontalValue =
            qFloor(boundingRect.left() + changeLeft * boundingRect.width());
        verticalValue =
            qFloor(boundingRect.top() + changeTop * boundingRect.height());
      }
    } else {
      if (index == m_currentPage - 1) {
        page->setVisible(true);
...
void DocumentView::prepareHighlight() { m_highlight->setVisible(false); }
...
void DocumentView::on_syncpage(int index, const QPointF &pagePos) {
  QPointF pos((pagePos.x()) / scaleFactor() / physicalDpiX() * 72.0,
              (pagePos.y()) / scaleFactor() / physicalDpiY() * 72.0);
...
void DocumentView::countWords() {
  ...
  for (int i = 0; i < m_document->pageCount(); ++i) {
    ...
    if (progress.wasCanceled())
      return;
    pagewords = 0;
    pdf_text = m_document->checkTextAt(i, m_pages.at(i)->boundingRect(),
                                       m_pages.at(i)->boundingRect());
  ...
  QMessageBox::information(this, "Texmaker",
                           tr("Number of words in the document") +
                               QString(" : ") + QString::number(numwords));
}
...
void DocumentView::pngExport(int page) {
  ...
  QString fn = QFileDialog::getSaveFileName(this, tr("Save As"), currentDir,
                                            "Png Image (*.png)");
  if (!fn.isEmpty()) {
    image.save(fn, "PNG");
  }
...
void DocumentView::countPageWords(int page) {
  ...
  int numwords = 0;
  int pagewords = 0;
  pdf_text = m_document->checkTextAt(page, m_pages.at(page)->boundingRect(),
                                     m_pages.at(page)->boundingRect());
  pdf_text = pdf_text.simplified();
  pagewords = pdf_text.count(" ");
  if (!pdf_text.isEmpty())
    pagewords++;
  numwords += pagewords;
  QMessageBox::information(this, "Texmaker",
                           tr("Number of words in the page") + QString(" : ") +
                               QString::number(numwords));
}
