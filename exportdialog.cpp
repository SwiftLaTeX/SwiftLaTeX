...
ExportDialog::ExportDialog(QWidget *parent, QString f, QString ep)
    : QDialog(parent) {
  setModal(true);
  ui.setupUi(this);
  filename = f;
  extra_path = ep;
  connect(ui.radioButton5, SIGNAL(toggled(bool)), ui.lineEditOptions,
...
void ExportDialog::runHtlatex() {
  ...
  if (ui.radioButton1->isChecked()) {
    ...
    commandline += "\"xhtml,ooffice\" \"ooffice/! -cmozhtf\" \"-coo\" "
                   "-interaction=nonstopmode";
  } else if (ui.radioButton4->isChecked()) {
    commandline += "\"xhtml,mozilla\" \" -cmozhtf\" \"-cvalidate\" "
                   "-interaction=nonstopmode";
  } else if (ui.radioButton4->isChecked()) {
...
void ExportDialog::readFromStderr() {
  ...
  t = t.simplified();
  if (!t.isEmpty())
    ui.plainTextEditResult->insertPlainText(t + "\n");
}
...
void ExportDialog::SlotEndProcess(int err) {
  ...
  QStringList extension =
      QString(".4,.4ct,.4tc,.aux,.dvi,.idv,.lg,.log,.tmp,.xref").split(",");
