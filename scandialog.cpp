...
ScanDialog::ScanDialog(QWidget *parent, const char *name) : QDialog(parent) {
  setWindowTitle(name);
  setModal(false);
  ui.setupUi(this);
  ...
  connect(scanthread, SIGNAL(fileCountChanged(int)), this,
          SLOT(setFileCount(int)));
  ui.pushButtonAbort->setEnabled(false);
  connect(ui.treeWidget, SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)),
          this, SLOT(ClickedOnResult(QTreeWidgetItem *, int)));
}
...
void ScanDialog::browse() {
  QString directory =
      QFileDialog::getExistingDirectory(this, tr("Find"), QDir::currentPath());
