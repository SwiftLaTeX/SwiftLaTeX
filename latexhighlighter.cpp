...
LatexHighlighter::LatexHighlighter(QTextDocument *parent, bool spelling,
  ...
  KeyWords =
      QString("section{,subsection{,subsubsection{,chapter{,part{,paragraph{,"
              "subparagraph{,section*{,subsection*{,subsubsection*{,chapter*{,"
              "part*{,paragraph*{,subparagraph*{,label{,includegraphics{,"
              "includegraphics[,includegraphics*{,includegraphics*[,include{,"
              "input{,begin{,end{")
          .split(",");
  KeyWordsGraphic =
      QString("void bool bool3 int real pair triple string").split(" ");
  KeyWordsGraphicBis =
      QString("and controls tension atleast curl if else while for do return "
              "break continue struct typedef new access import unravel from "
              "include quote static public private restricted this explicit "
...
void LatexHighlighter::SetEditor(LatexEditor *ed) { editor = ed; }
...
void LatexHighlighter::highlightBlock(const QString &text) {
  ...
  if (!isGraphic) {
    ...
    while (i < text.length()) {
      ...
      case StateStandard: {
        ...
        if (tmp == '\\') {
          ...
          if (poslab != -1) {
            for (k = i - buffer.length() + poslab; k < i; k++) {
          ...
          if (posbib != -1) {
            ...
            if ((posbib2 != -1) && (posbib2 > posbib))
              setFormat(posbib, posbib2 - posbib, structFormat);
            for (k = posbib; k < i; k++) {
              if (k > 0 && k < text.length())
                blockData->code[k] = 1;
            }
      ...
      case StateCommand: {
        ...
        if (rxverb.exactMatch(buffer)) {
          ...
          state = StateStandard;
          if (buffer.length() > 0) {
            for (QStringList::Iterator it = KeyWords.begin();
                 it != KeyWords.end(); ++it) {
              if ((*it).indexOf(buffer) != -1) {
                if (*it != "begin{" && *it != "end{")
                  setFormat(i - buffer.length(), buffer.length(), structFormat);
                else
      ...
      case StateGraphic: {
        ...
        if (tmp == '\\') {
          ...
          setFormat(i, 1, ColorVerbatim);
          buffer = QString::null;
        } else if (isWordSeparator(tmp) && tmp != '"' && tmp != '.' &&
                   tmp != '-') {
          blockData->code[i] = 1;
          setFormat(i, 1, ColorVerbatim);
          buffer = QString::null;
    ...
    for (int j = 0; j < structure.count(); j++) {
      if ((structure.at(j).cursor.block().blockNumber() ==
           currentBlock().blockNumber()) &&
          (structure.at(j).type != 0)) {
        int offwidth = 0;
        QTextCursor curs =
            editor->document()->find("{", structure.at(j).cursor.position());
        if (!curs.isNull() &&
            (curs.position() <
             currentBlock().length() + currentBlock().position())) {
          offwidth = curs.position() - structure.at(j).cursor.position() +
                     structure.at(j).item.length() + 1;
        }
        if (offwidth > 0)
          setFormat(structure.at(j).cursor.position() -
                        currentBlock().position(),
                    offwidth, structFormat);
      }
    }
    //}
    ////////////////////
    ...
    if (state == StateComment) {
      ...
      while (i < text.length()) {
        ...
        if ((buffer.length() > 0) &&
            (format(i - buffer.length()).foreground() == brushcomment)) {
    ...
    if (state == StateGraphic || state == StateGraphicCommand ||
        state == StateGraphicComment || state == StateGraphicAsy ||
        state == StateGraphicAsyCommand || state == StateGraphicAsyComment) {
      QRegExp number("[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?");
      int index = number.indexIn(text);
      while (index >= 0) {
        int length = number.matchedLength();
        if (blockData->code[index] != 1)
          setFormat(index, length, ColorNumberGraphic);
      ...
      if (state == StateGraphicAsy || state == StateGraphicAsyCommand ||
        ...
        while (index >= 0) {
          ...
          if (blockData->code[index] != 1)
            setFormat(index, length, ColorKeyword);
        ...
        while (i < text.length()) {
          ...
          if ((buffer.length() > 0) &&
              (format(i - buffer.length()).foreground() == brushverbatim)) {
            for (QStringList::Iterator it = KeyWordsGraphic.begin();
                 it != KeyWordsGraphic.end(); ++it) {
              if (*it == buffer) {
                setFormat(i - buffer.length(), buffer.length(),
                          ColorKeywordGraphic);
              }
            }
            for (QStringList::Iterator it = KeyWordsGraphicBis.begin();
                 it != KeyWordsGraphicBis.end(); ++it) {
              if (*it == buffer) {
      ...
      } else if ((text.indexOf("pspicture") == -1) &&
                 (text.indexOf("tikzpicture") == -1)) {
        QRegExp expression("\\\\[A-Za-z]+");
        index = expression.indexIn(text);
        while (index >= 0) {
          int length = expression.matchedLength();
          if ((format(index).foreground() != brushmath) &&
              (format(index).foreground() != brushcomment))
            setFormat(index, length, ColorKeywordGraphic);
          index = expression.indexIn(text, index + length);
        }
      }
    }

    if (pChecker && state != StateGraphic && state != StateGraphicCommand &&
        state != StateGraphicComment && state != StateGraphicAsy &&
        state != StateGraphicAsyCommand && state != StateGraphicAsyComment) {
      i = 0;
      int check;
      ...
      while (i < text.length()) {
        ...
        if ((buffer.length() > 1) && (!ignoredwordList.contains(buffer)) &&
            (!hardignoredwordList.contains(buffer))) {
          if (format(i - buffer.length() - offset).foreground() ==
              brushverbatim)
            spellingErrorFormat.setForeground(brushverbatim);
          else if (format(i - buffer.length() - offset).foreground() ==
                   brushkeyword)
            spellingErrorFormat.setForeground(brushkeyword);
          else
            spellingErrorFormat.setForeground(brushstandard);
          if (format(i - buffer.length() - offset).fontWeight() ==
              QFont::Bold) {
            spellingErrorFormat.setFontWeight(QFont::Bold);
            spellingErrorFormat.setFontUnderline(true);
          } else
            spellingErrorFormat.setFontWeight(QFont::Normal);
          encodedString = codec->fromUnicode(buffer);
          check = pChecker->spell(encodedString.data());
          if (!check) {
            if (checkSpelling)
              setFormat(i - buffer.length() - offset, buffer.length(),
                        spellingErrorFormat);
            for (int k = i - buffer.length() - offset; k < i - offset; k++)
              blockData->misspelled[k] = true;
          }
        }
        i++;
      }
...
void LatexHighlighter::setModeGraphic(bool m) { isGraphic = m; }
