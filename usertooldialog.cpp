...
UserToolDialog::UserToolDialog(QWidget *parent, QString name, QStringList names,
...
void UserToolDialog::init() {
  ...
  connect(ui.itemEdit, SIGNAL(textChanged(const QString &)), this,
          SLOT(updateItem()));
}
...
void UserToolDialog::updateItem() {
  ...
  ui.listWidget->item(i)->setText("Command " + QString::number(i + 1) + " : " +
                                  ui.itemEdit->text());
}
