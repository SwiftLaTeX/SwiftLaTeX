...
UserQuickDialog::UserQuickDialog(QWidget *parent, QStringList usualNames,
                                 QStringList usualCommands)
    : QDialog(parent) {
  ui.setupUi(this);
  ...
  for (int i = 0; i < usualNames.count(); i++)
    cmdmap.insert(usualNames.at(i), usualCommands.at(i));
}
...
void UserQuickDialog::actionDown() {
  int current = ui.listWidget->currentRow();
  if (current >= ui.listWidget->count() - 1 || current < 0)
    return;
  QListWidgetItem *item = ui.listWidget->item(current)->clone();
  delete ui.listWidget->item(current);
  ui.listWidget->insertItem(current + 1, item);
