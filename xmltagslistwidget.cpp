...
XmlTagsListWidget::XmlTagsListWidget(QWidget *parent, QString file)
    : QListWidget(parent) {
  QFile tagsFile(file);
...
void XmlTagsListWidget::addListWidgetItems(const xmlTagList &tagList) {
  ...
  for (int i = 0; i < tagList.tags.size(); ++i) {
    ...
    QString role = tagList.tags.at(i).tag + "#" + tagList.tags.at(i).dx + "#" +
                   tagList.tags.at(i).dy;
    item->setData(Qt::UserRole, role);
    if (tagList.tags.at(i).type == 0) {
