...
PresentationView::PresentationView(QMutex *mutex, QPdfDocument *document)
    : QWidget(), m_mutex(0), m_document(0), m_numberOfPages(-1),
      m_currentPage(-1), m_returnToPage(-1),
      //    m_links(),
      m_scaleFactor(1.0), m_normalizedTransform(), m_boundingRect(), m_image(),
      m_render(0) {
...
int PresentationView::numberOfPages() const { return m_numberOfPages; }

int PresentationView::currentPage() const { return m_currentPage; }

void PresentationView::previousPage() { jumpToPage(currentPage() - 1, false); }

void PresentationView::nextPage() { jumpToPage(currentPage() + 1, false); }

void PresentationView::firstPage() { jumpToPage(1); }

void PresentationView::lastPage() { jumpToPage(numberOfPages()); }
...
void PresentationView::startRender() {
  if (!m_render->isRunning()) {
    m_render->setFuture(QtConcurrent::run(this, &PresentationView::render,
                                          m_currentPage - 1, m_scaleFactor));
  }
}
...
void PresentationView::on_render_finished() { update(); }
...
void PresentationView::paintEvent(QPaintEvent *event) {
  ...
  painter.fillRect(rect(),
                   QBrush(PageItem::invertColors() ? Qt::white : Qt::black));
...
void PresentationView::mouseMoveEvent(QMouseEvent *event) {
  ...
  //             //QToolTip::showText(event->globalPos(), tr("Go to page
  //             %1.").arg(link->destination().pageNumber()));
  //
  //             return;
  //         }
  //     }
...
void PresentationView::prepareView() {
  ...
  //         {
  //             Poppler::LinkGoto* linkGoto = static_cast< Poppler::LinkGoto*
  //             >(link);
  //
...

  {
    ...
    m_normalizedTransform.translate(m_boundingRect.left(),
                                    m_boundingRect.top());
    m_normalizedTransform.scale(m_boundingRect.width(),
...
void PresentationView::render(int index, qreal scaleFactor) {
  ...
  // QImage image = m_document->render(index,QSize(size.width()*
  // physicalDpiX()/72,size.height()* physicalDpiY()/72));
