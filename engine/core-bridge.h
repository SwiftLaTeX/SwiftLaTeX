...
typedef struct tt_bridge_api_t {
  ...
  int (*get_data_md5)(void *context, char const *data, size_t len,
                      char *digest);

  rust_output_handle_t (*output_open)(void *context, char const *path,
                                      int is_gz);
  rust_output_handle_t (*output_open_stdout)(void *context);
  int (*output_putc)(void *context, rust_output_handle_t handle, int c);
  size_t (*output_write)(void *context, rust_output_handle_t handle,
                         const char *data, size_t len);
  int (*output_flush)(void *context, rust_output_handle_t handle);
  int (*output_close)(void *context, rust_output_handle_t handle);

  rust_input_handle_t (*input_open)(void *context, char const *path,
                                    tt_input_format_type format, int is_gz);

  size_t (*input_get_size)(void *context, rust_input_handle_t handle);
  size_t (*input_seek)(void *context, rust_input_handle_t handle,
                       ssize_t offset, int whence, int *internal_error);
  ssize_t (*input_read)(void *context, rust_input_handle_t handle, char *data,
                        size_t len);
