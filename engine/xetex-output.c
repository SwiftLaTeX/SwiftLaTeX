...
void print(int32_t s) {
  ...
  for (pool_pointer i = str_start[pool_idx]; i < str_start[pool_idx + 1]; i++) {
    if ((str_pool[i] >= 0xD800) && (str_pool[i] < 0xDC00) &&
        (i + 1 < str_start[pool_idx + 1]) && (str_pool[i + 1] >= 0xDC00) &&
        (str_pool[i + 1] < 0xE000)) {
      print_char(0x10000 + (str_pool[i] - 0xD800) * 1024 + str_pool[i + 1] -
                 0xDC00);
      i++;
    } else {
...
void print_nl(str_number s) {
  if (((term_offset > 0) && (odd(selector))) ||
      ((file_offset > 0) && (selector >= SELECTOR_LOG_ONLY)))
    print_ln();
  print(s);
}
...
void print_file_name(int32_t n, int32_t a, int32_t e) {
  ...
  if (a != 0) {
    ...
    while (((!must_quote) || (quote_char == 0)) &&
           (j < str_start[(a + 1) - 0x10000])) {
      if (str_pool[j] == ' ')
        must_quote = true;
