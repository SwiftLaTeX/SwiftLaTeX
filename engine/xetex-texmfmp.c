...
void get_date_and_time(time_t source_date_epoch, int32_t *minutes, int32_t *day,
...
static void checkpool_pointer(pool_pointer pool_ptr, size_t len) {
...
char *gettexstring(str_number s) {
...
static int compare_paths(const char *p1, const char *p2) {
  int ret;
...
bool is_new_source(str_number srcfilename, int lineno) {
  char *name = gettexstring(srcfilename);
  return (compare_paths(name, last_source_name) != 0 || lineno != last_lineno);
}

void remember_source_info(str_number srcfilename, int lineno) {
  free(last_source_name);
  last_source_name = gettexstring(srcfilename);
  last_lineno = lineno;
}

pool_pointer make_src_special(str_number srcfilename, int lineno) {
