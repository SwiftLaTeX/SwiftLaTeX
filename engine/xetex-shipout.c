...
static int32_t reverse(int32_t this_box, int32_t t, scaled_t *cur_g,
                       double *cur_glue);
...
static inline void dvi_out(eight_bits c) {
  dvi_buf[dvi_ptr++] = c;
...
void ship_out(int32_t p) {
  ...
  unsigned char l;
  const char *output_comment = "swiftlatex";
...
//  synctex_sheet(INTPAR(mag));
  ...
  if (BOX_height(p) > MAX_HALFWORD || BOX_depth(p) > MAX_HALFWORD ||
  ...
  cur_v = BOX_height(p) +
          DIMENPAR(v_offset); /*"Does this need changing for upwards mode???"*/
  temp_ptr = p;
...
static void hlist_out(void) {
        ...
        check_next:
          ...
          while (q != TEX_NULL && !is_char_node(q) &&
                 (NODE_type(q) == PENALTY_NODE || NODE_type(q) == INS_NODE ||
                  NODE_type(q) == MARK_NODE || NODE_type(q) == ADJUST_NODE ||
              ...
              if (GLUE_NODE_glue_ptr(q) == font_glue[NATIVE_NODE_font(r)]) {
                ...
                while (q != TEX_NULL && !is_char_node(q) &&
                       (NODE_type(q) == PENALTY_NODE ||
                        NODE_type(q) == INS_NODE || NODE_type(q) == MARK_NODE ||
          ...
          if (p != r) {
            ...
            while (true) {
              if (NODE_type(q) == WHATSIT_NODE) {
                if (NODE_subtype(q) == NATIVE_WORD_NODE ||
                    NODE_subtype(q) == NATIVE_WORD_NODE_AT) {
                ...
                if (g_sign != NORMAL) {
                  if (g_sign == STRETCHING) {
                    if (GLUE_SPEC_stretch_order(g) == g_order)
                      k += tex_round(BOX_glue_set(this_box) *
                                     GLUE_SPEC_stretch(g));
                  } else {
            ...
            for (j = 0; j < cur_length(); j++)
              NATIVE_NODE_text(q)[j] =
                  str_pool[str_start[str_ptr - TOO_BIG_CHAR] + j];
  ...
  base_line = cur_v;
  prev_p = this_box +
           5; /* this is list_offset, the offset of the box list pointer */
  ...
  if (cur_dir == RIGHT_TO_LEFT && BOX_lr_mode(this_box) != REVERSED) {
    ...
    SYNCTEX_tag(p, MEDIUM_NODE_SIZE) =
        0; /* "SyncTeX: do nothing, it is too late" */
    LLIST_link(prev_p) = p;
    cur_h = 0;
        ...
        if (NODE_subtype(p) >= A_LEADERS) {
          ...
          if (leader_wd > 0 && rule_wd > 0) {
            ...
            while (cur_h + leader_wd <= edge) {
              /*650: "Output a leader box at cur_h, then advance cur_h by
               * leader_wd + lx" */
      ...
      case MATH_NODE:
        ...
        if (odd(NODE_subtype(p))) { /* <= this is end_LR(p) */
          ...
          if (MATH_NODE_lr_dir(p) != cur_dir) {
            ...
            LLIST_link(p) =
                reverse(this_box, new_edge(1 - cur_dir, 0), &cur_g, &cur_glue);
      ...
      if (is_char_node(p)) {
        ...
        case WHATSIT_NODE:
          if (NODE_subtype(p) == NATIVE_WORD_NODE ||
              NODE_subtype(p) == NATIVE_WORD_NODE_AT ||
              NODE_subtype(p) == GLYPH_NODE || NODE_subtype(p) == PIC_NODE ||
...
static void pic_out(int32_t p) {
  ...
  pool_ptr =
      str_start[str_ptr - TOO_BIG_CHAR]; /* discard the string we just made */
}
...
void finalize_dvi_file(void) {
  ...
  if (cur_s == -2)
    /* This happens when the DVI gets too big; a message has already been
     * printed */
    return;
