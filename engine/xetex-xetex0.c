...
static void int_error(int32_t n) {
  print_cstr(" (");
  print_int(n);
  print_char(')');
  error();
...
void copy_native_glyph_info(int32_t src, int32_t dest) {
  ...
  if (NATIVE_NODE_glyph_info_ptr(src) != NULL) {
    ...
    NATIVE_NODE_glyph_info_ptr(dest) =
        xmalloc_array(char, glyph_count *NATIVE_GLYPH_INFO_SIZE);
    memcpy(NATIVE_NODE_glyph_info_ptr(dest), NATIVE_NODE_glyph_info_ptr(src),
...
void print_delimiter(int32_t p) {
  int32_t a;
  a = (mem[p].b16.s3 % 256) * 256 +
      (mem[p].b16.s2 + (mem[p].b16.s3 / 256) * 65536L);
  a = a * 4096 + (mem[p].b16.s1 % 256) * 256 +
      (mem[p].b16.s0 + (mem[p].b16.s1 / 256) * 65536L);
  if (a < 0)
    print_int(a);
  else
    print_hex(a);
    ...
    if (is_char_node(p)) {
      ...
      case WHATSIT_NODE:
        ...
        case OPEN_NODE:
          ...
          print_char('=');
          print_file_name(mem[p + 1].b32.s1, mem[p + 2].b32.s0,
                          mem[p + 2].b32.s1);
          break;
        case WRITE_NODE:
  ...
  while (p != TEX_NULL) {
    ...
    if (is_char_node(p)) {
      ...
      case WHATSIT_NODE:
        ...
        case PIC_NODE:
        case PDF_NODE:
          free_node(p, (PIC_NODE_SIZE +
                        (mem[p + 4].b16.s1 + sizeof(memory_word) - 1) /
                            sizeof(memory_word)));
          break;
        case PDF_SAVE_POS_NODE:
          free_node(p, SMALL_NODE_SIZE);
  ...
  while (p != TEX_NULL) {
    ...
    if (is_char_node(p)) {
      ...
      case WHATSIT_NODE:
        ...
        case PDF_NODE:
          words =
              (PIC_NODE_SIZE + (mem[p + 4].b16.s1 + sizeof(memory_word) - 1) /
                                   sizeof(memory_word));
          r = get_node(words);
          break;
        case PDF_SAVE_POS_NODE:
...
void show_context(void) {
  ...
  while (true) {
    ...
    if ((base_ptr == input_ptr) || bottom_line ||
        (nn < INTPAR(error_context_lines))) { /*324: */
      if ((base_ptr == input_ptr) || (cur_input.state != TOKEN_LIST) ||
        ...
        if (cur_input.state != TOKEN_LIST) {
          ...
          if (cur_input.index < MACRO)
            ...
            show_token_list(mem[cur_input.start].b32.s1, cur_input.loc,
                            100000L);
        }
        selector = old_setting;
      ...
      if (INTPAR(tracing_macros) > 1) {
        ...
        default:
          print_cmd_chr(ASSIGN_TOKS,
                        t + LOCAL_BASE + LOCAL__output_routine - OUTPUT_TEXT);
          break;
        }
        print_cstr("->");
        token_show(p);
...
restart:
  ...
  texswitch:
    if (cur_input.loc <= cur_input.limit) {
      ...
      if (cur_chr >= 0xD800 && cur_chr < 0xDC00 &&
          cur_input.loc <= cur_input.limit && buffer[cur_input.loc] >= 0xDC00 &&
          buffer[cur_input.loc] < 0xE000) {
    ...
    reswitch:
      ...
      switch (cur_input.state + cur_cmd) { /*357:*/
        ANY_STATE_PLUS(IGNORE)
            : case SKIP_BLANKS + SPACER : case NEW_LINE +
                                          SPACER : goto texswitch;
        break;

        ANY_STATE_PLUS(ESCAPE) : if (cur_input.loc > cur_input.limit) {
          cur_cs = NULL_CS;
        }
        else {
        start_cs:
          ...
          if (cat == LETTER && k <= cur_input.limit) { /*368:*/
            ...
            if (cat == SUP_MARK && buffer[k] == cur_chr &&
                k < cur_input.limit) {
              ...
              while (sup_count < 6 &&
                     k + 2 * sup_count - 2 <= cur_input.limit &&
      ...
      found:
        ...
        ANY_STATE_PLUS(ACTIVE_CHAR) : cur_cs = cur_chr + 1;
        cur_cmd = eqtb[cur_cs].b16.s1;
        cur_chr = eqtb[cur_cs].b32.s1;
        cur_input.state = MID_LINE;
        if (cur_cmd >= OUTER_CALL)
        ...
        ANY_STATE_PLUS(SUP_MARK) : if (cur_chr == buffer[cur_input.loc]) {
          if (cur_input.loc < cur_input.limit) {
            sup_count = 2;

            while (sup_count < 6 &&
                   cur_input.loc + 2 * sup_count - 2 <= cur_input.limit &&
      ...
      not_exp:
        ...
        ANY_STATE_PLUS(INVALID_CHAR)
            : if (file_line_error_style_p) print_file_line();
        else print_nl_cstr("! ");
      ...
      case MID_LINE + CAR_RET:
        ...
        ANY_STATE_PLUS(COMMENT)
            : case SKIP_BLANKS + CAR_RET : cur_input.loc = cur_input.limit + 1;
        goto texswitch;
        break;
      ...
      case NEW_LINE + RIGHT_BRACE:
        ...
        ADD_DELIMS_TO(SKIP_BLANKS)
            : ADD_DELIMS_TO(NEW_LINE) : cur_input.state = MID_LINE;
        break;
      ...
      if (cur_input.name > 17) { /*374:*/
        ...
        if (!force_eof) {
          if (cur_input.name <= 19) {
            ...
            } else if (LOCAL(every_eof) != TEX_NULL &&
        ...
        if (!used_tectonic_coda_tokens &&
            LOCAL(TectonicCodaTokens) != TEX_NULL) {
  ...
  } else if (cur_input.loc !=
             TEX_NULL) { /* if we're inputting from a non-null token list: */
            ...
            if (cur_tok == par_token) {
              if (long_state != LONG_CALL) { /*414:*/
                if (long_state == CALL) {
                  ...
                  print_cstr(" was complete");
                  help_ptr = 3;
                  help_line[2] = "I suspect you've forgotten a `}', causing me "
                                 "to apply this";
                  help_line[1] =
                      "control sequence to too much text. How can we recover?";
                  help_line[0] = "My plan is to forget the whole thing and "
                                 "hope for the best.";
                  back_error();
                }
...
reswitch:
  ...
  case 17:
    if (cur_chr == 2) {
      ...
      c = set_class(cur_val / 4096) + set_family((cur_val % 4096) / 256) +
          (cur_val % 256);
    }
    break;
  case 69: {
    c = set_class(cur_chr / 4096) + set_family((cur_chr % 4096) / 256) +
        (cur_chr % 256);
  } break;
  case 70:
    c = cur_chr;
  ...
  if ((math_class(c) == 7) &&
      ((INTPAR(cur_fam) >= 0) && (INTPAR(cur_fam) < NUMBER_MATH_FAMILIES)))
...
void set_math_char(int32_t c) {
  ...
  if (math_char(c) == ACTIVE_MATH_CHAR) { /*1187: */
    ...
    if (math_class(c) == 7) {
      if (((INTPAR(cur_fam) >= 0) && (INTPAR(cur_fam) < NUMBER_MATH_FAMILIES)))
...
void scan_four_bit_int(void) {
  ...
  if ((cur_val < 0) || (cur_val > 15)) {
    ...
    }
    {
      help_ptr = 2;
      help_line[1] =
          "Since I expected to read a number between 0 and 15,"; /* ... "between
                                                                    0 and 15" */
      help_line[0] = "I changed this one to zero.";
    }
    int_error(cur_val);
...
void scan_something_internal(small_number level, bool negative) {
  ...
  case DEF_CODE:
    ...
    if (m == MATH_CODE_BASE) {
      ...
      cur_val1 = math_class(cur_val1) * 0x1000 + math_fam(cur_val1) * 0x100 +
                 math_char(cur_val1);
      cur_val = cur_val1;
      cur_val_level = INT_VAL;
  ...
  case DEF_FONT:
    if (level != TOK_VAL) {
      ...
      if (cur_cmd < ASSIGN_TOKS) {
        ...
        cur_ptr = cur_val;
        scan_char_class_not_ignored();
        find_sa_element(INTER_CHAR_VAL, cur_ptr * CHAR_CLASS_LIMIT + cur_val,
                        false);
        if (cur_ptr == TEX_NULL)
          cur_val = TEX_NULL;
        else
      ...
      if (m >= XETEX_DIM) {
        ...
        case XETEX_OT_FEATURE_CODE:
          ...
          if (font_area[n] == OTGR_FONT_FLAG &&
            ...
            scan_int();
            cur_val =
                ot_font_get_3(m - 14, font_layout_engine[n], k, kk, cur_val);
          } else {
            not_ot_font_error(LAST_ITEM, m, n);
            cur_val = -1;
          }
...
static scaled_t round_decimals(small_number k) {
  int32_t a = 0;
...
void xetex_scan_dimen(bool mu, bool inf, bool shortcut, bool requires_units) {
  ...
  if (requires_units) {
    ...
    if (scan_keyword("em"))
      ...
      v = font_info[X_HEIGHT_CODE + param_base[eqtb[CUR_FONT_LOC].b32.s1]]
              .b32.s1;
    else
      goto not_found;
  ...
  found:
    cur_val =
        mult_and_add(save_cur_val, v, xn_over_d(v, f, 65536L), MAX_HALFWORD);
    goto attach_sign;
...
void scan_decimal(void) { xetex_scan_dimen(false, false, false, false); }
...
found: /*1572:*/ /*424:*/
  ...
  if ((l == INT_VAL) || (s > EXPR_SUB)) {
    ...
    if ((abs(mem[f + 1].b32.s1) > MAX_HALFWORD) ||
        (abs(mem[f + 2].b32.s1) > MAX_HALFWORD) ||
  ...
  if (o > EXPR_SUB)
    ...
    if (r == EXPR_NONE)
      ...
      mem[e + 1].b32.s1 = add_or_sub(mem[e + 1].b32.s1, mem[t + 1].b32.s1,
                                     MAX_HALFWORD, r == EXPR_SUB);
      if (mem[e].b16.s1 == mem[t].b16.s1)
        mem[e + 2].b32.s1 = add_or_sub(mem[e + 2].b32.s1, mem[t + 2].b32.s1,
                                       MAX_HALFWORD, r == EXPR_SUB);
      ...
      if (mem[e].b16.s0 == mem[t].b16.s0)
        mem[e + 3].b32.s1 = add_or_sub(mem[e + 3].b32.s1, mem[t + 3].b32.s1,
                                       MAX_HALFWORD, r == EXPR_SUB);
...
void scan_normal_glue(void) { scan_glue(GLUE_VAL); }

void scan_mu_glue(void) { scan_glue(MU_VAL); }
...
int32_t str_toks_cat(pool_pointer b, small_number cat) {
  ...
  while (k < pool_ptr) {
    ...
    if ((t == ' ') && (cat == 0))
      ...
      if ((t >= 0xD800) && (t < 0xDC00) && (k + 1 < pool_ptr) &&
          (str_pool[k + 1] >= 0xDC00) && (str_pool[k + 1] < 0xE000)) {
        k++;
...
int32_t str_toks(pool_pointer b) { return str_toks_cat(b, 0); }
...
void conv_toks(void) {
  ...
  case XETEX_FEATURE_NAME_CODE:
    ...
    fnt = cur_val;
    if (font_area[fnt] == AAT_FONT_FLAG ||
        (font_area[fnt] == OTGR_FONT_FLAG &&
         usingGraphite(font_layout_engine[fnt]))) {
      scan_int();
      arg1 = cur_val;
      arg2 = 0;
    } else {
  ...
  case XETEX_SELECTOR_NAME_CODE:
    if (font_area[fnt] == AAT_FONT_FLAG)
      aat_print_font_name(c, font_layout_engine[fnt], arg1, arg2);
    else if (font_area[fnt] == OTGR_FONT_FLAG &&
             usingGraphite(font_layout_engine[fnt]))
  ...
  case LEFT_MARGIN_KERN_CODE:
    ...
    while (p != TEX_NULL &&
           ((p < hi_mem_min &&
             (NODE_type(p) == INS_NODE || NODE_type(p) == MARK_NODE ||
              NODE_type(p) == ADJUST_NODE || NODE_type(p) == PENALTY_NODE ||
              (NODE_type(p) == DISC_NODE && mem[p + 1].b32.s0 == TEX_NULL &&
               mem[p + 1].b32.s1 == TEX_NULL && mem[p].b16.s0 == 0) ||
              (NODE_type(p) == MATH_NODE && mem[p + 1].b32.s1 == 0) ||
              (NODE_type(p) == KERN_NODE &&
               (mem[p + 1].b32.s1 == 0 || mem[p].b16.s0 == NORMAL)) ||
              (NODE_type(p) == GLUE_NODE && mem[p + 1].b32.s0 == 0) ||
              (NODE_type(p) == HLIST_NODE && mem[p + 1].b32.s1 == 0 &&
               mem[p + 3].b32.s1 == 0 && mem[p + 2].b32.s1 == 0 &&
               mem[p + 5].b32.s1 == TEX_NULL))) ||
            (p < hi_mem_min && NODE_type(p) == GLUE_NODE &&
             mem[p].b16.s0 == (GLUE_PAR__left_skip + 1))))
      p = LLIST_link(p);

    if (p != TEX_NULL && p < hi_mem_min && NODE_type(p) == MARGIN_KERN_NODE &&
        mem[p].b16.s0 == 0)
      print_scaled(mem[p + 1].b32.s1);
    else
      print('0');
           ...
           ((p < hi_mem_min &&
             (NODE_type(p) == INS_NODE || NODE_type(p) == MARK_NODE ||
              ...
              (NODE_type(p) == HLIST_NODE && mem[p + 1].b32.s1 == 0 &&
               ...
               mem[p + 5].b32.s1 == TEX_NULL))) ||
            (p < hi_mem_min && NODE_type(p) == GLUE_NODE &&
             mem[p].b16.s0 == (GLUE_PAR__right_skip + 1))))
...
void end_name(void) {
  ...
  if (area_delimiter == 0) {
    ...
    cur_area = str_ptr;
    str_start[(str_ptr + 1) - 65536L] =
        str_start[str_ptr - TOO_BIG_CHAR] + area_delimiter;
    str_ptr++;
  ...
  if (ext_delimiter == 0) {
    ...
    cur_name = str_ptr;
    str_start[(str_ptr + 1) - 65536L] =
        str_start[str_ptr - TOO_BIG_CHAR] + ext_delimiter - area_delimiter - 1;
    str_ptr++;
...
void pack_file_name(str_number n, str_number a, str_number e) {
  ...
  char *work_buffer =
      xmalloc_array(UTF8_code, (length(a) + length(n) + length(e)) * 3 + 1);
  work_buffer[0] = '\0';
...
void start_input(const char *primary_input_name) {
  ...
  if (!u_open_in(&input_file[cur_input.index], format, "rb",
                 INTPAR(xetex_default_input_mode),
  ...
  if (term_offset + length(full_source_filename_stack[in_open]) >
      max_print_line - 2)
    print_ln();
  else if (term_offset > 0 || file_offset > 0)
    print_char(' ');
...
void char_warning(internal_font_number f, int32_t c) {
...

  {
    ...
    s = make_string();
    chr = gettexstring(s);
    str_ptr--; /* this is the "flush_string" macro which discards the most
                  recent string */
    pool_ptr = str_start[str_ptr - 0x10000];

    ttstub_issue_warning("could not represent character \"%s\" in font \"%s\"",
                         chr, fn);
    ...
    if (!gave_char_warning_help) {
      ttstub_issue_warning("  you may need to load the `fontspec` package and "
                           "use (e.g.) \\setmainfont to");
      ttstub_issue_warning("  choose a different font that covers the "
                           "unrepresentable character(s)");
...
int32_t new_native_word_node(internal_font_number f, int32_t n) {
  ...
  l = NATIVE_NODE_SIZE +
      (n * sizeof(UTF16_code) + sizeof(memory_word) - 1) / sizeof(memory_word);
  q = get_node(l);
...
int32_t new_native_character(internal_font_number f, UnicodeScalar c) {
  ...
  if (font_mapping[f] != NULL) {
    ...
    len = apply_mapping(font_mapping[f],
...
void font_feature_warning(const void *featureNameP, int32_t featLen,
                          const void *settingNameP, int32_t setLen) {
  begin_diagnostic();
...
void font_mapping_warning(const void *mappingNameP, int32_t mappingNameLen,
                          int32_t warningType) {
  begin_diagnostic();
  if (warningType == 0)
...
internal_font_number load_native_font(int32_t u, str_number nom,
                                      str_number aire, scaled_t s) {
  ...
  for (f = FONT_BASE + 1; f <= font_ptr; f++) {
    if (font_area[f] == native_font_type_flag &&
        str_eq_str(font_name[f], full_name) && font_size[f] == actual_size) {
  ...
  if (native_font_type_flag == OTGR_FONT_FLAG &&
      isOpenTypeMathFont(font_engine))
    num_font_dimens = 65; /* = first_math_fontdimen (=10) + lastMathConstant (=
    ...
    do {
      ...
      if (offs > 0) {
        if (prevOffs != 0) {
          if (use_penalty) {
            mem[cur_list.tail].b32.s1 =
                new_penalty(INTPAR(xetex_linebreak_penalty));
            cur_list.tail = LLIST_link(cur_list.tail);
          }
          if (use_skip) {
            mem[cur_list.tail].b32.s1 =
                new_param_glue(GLUE_PAR__xetex_linebreak_skip);
            cur_list.tail = LLIST_link(cur_list.tail);
          }
        }
        mem[cur_list.tail].b32.s1 =
        ...
        cur_list.tail = LLIST_link(cur_list.tail);
        {
          ...
          if (i <= for_end)
            do
              NATIVE_NODE_text(cur_list.tail)[i - prevOffs] =
                  native_text[s + i];
            while (i++ < for_end);
        }
        set_native_metrics(cur_list.tail,
...
int32_t get_tracing_fonts_state(void) { return INTPAR(xetex_tracing_fonts); }

internal_font_number read_font_info(int32_t u, str_number nom, str_number aire,
                                    scaled_t s) {
  font_index k;
  bool name_too_long;
  bool file_opened;
...
scaled_t char_pw(int32_t p, small_number side) {
  ...
  if (p == TEX_NULL)
    return 0;
  if (((p != TEX_NULL && (!(is_char_node(p))) &&
        (NODE_type(p) == WHATSIT_NODE) &&
        (NODE_subtype(p) == NATIVE_WORD_NODE ||
         NODE_subtype(p) == NATIVE_WORD_NODE_AT)))) {
    if (NATIVE_NODE_glyph_info_ptr(p) != NULL) {
      f = NATIVE_NODE_font(p);
      return round_xn_over_d(font_info[QUAD_CODE + param_base[f]].b32.s1,
                             get_native_word_cp(p, side), 1000);
    } else {
      return 0;
    }
  }
  if ((((p) != TEX_NULL && (!(is_char_node(p))) &&
        (NODE_type(p) == WHATSIT_NODE) && (mem[p].b16.s0 == GLYPH_NODE)))) {
    f = mem[p + 4].b16.s2;
    return round_xn_over_d(font_info[QUAD_CODE + param_base[f]].b32.s1,
                           get_cp_code(f, mem[p + 4].b16.s1, side), 1000);
  }
  if (!(is_char_node(p))) {
...
int32_t hpack(int32_t p, scaled_t w, small_number m) {
          ...
          if ((pp != mem[p].b32.s1)) {
            ...
            do {
              if (NODE_type(ppp) == WHATSIT_NODE) {
                ...
                if (k <= for_end)
                  do {
                    NATIVE_NODE_text(pp)[total_chars] =
                        NATIVE_NODE_text(ppp)[k];
                    total_chars++;
                  } while (k++ < for_end);
              }
              ppp = LLIST_link(ppp);
  ...
  if (x == 0) {
    ...
    if ((total_shrink[o] < -(int32_t)x) && (o == NORMAL) &&
...
void append_to_vlist(int32_t b) {
  ...
  if (cur_list.aux.b32.s1 > IGNORE_DEPTH) {
    if (upwards)
      d = mem[GLUEPAR(baseline_skip) + 1].b32.s1 - cur_list.aux.b32.s1 -
          mem[b + 2].b32.s1;
    else
      d = mem[GLUEPAR(baseline_skip) + 1].b32.s1 - cur_list.aux.b32.s1 -
          mem[b + 3].b32.s1;
...
void show_info(void) { show_node_list(mem[temp_ptr].b32.s0); }
...
void init_row(void) {
  ...
  {
    mem[cur_list.tail].b32.s1 =
        new_glue(mem[mem[ALIGN_HEAD].b32.s1 + 1].b32.s0);
    cur_list.tail = LLIST_link(cur_list.tail);
  }
...
bool fin_col(void) {
  ...
  if (mem[cur_align + 5].b32.s0 != SPAN_CODE) {
    ...
    {
      mem[cur_list.tail].b32.s1 =
          new_glue(mem[mem[cur_align].b32.s1 + 1].b32.s0);
      cur_list.tail = LLIST_link(cur_list.tail);
    }
...
void fin_align(void) {
  ...
  if (cur_list.mode == -1) {
    ...
    p = hpack(mem[ALIGN_HEAD].b32.s1, save_stack[save_ptr + 1].b32.s1,
    ...
    p = vpackage(mem[ALIGN_HEAD].b32.s1, save_stack[save_ptr + 1].b32.s1,
                 save_stack[save_ptr + 0].b32.s1, MAX_HALFWORD);
    q = mem[mem[ALIGN_HEAD].b32.s1].b32.s1;
    do {
          ...
          if (cur_list.mode == -1) { /*839: */
            ...
            if (t == mem[r + 1].b32.s1) {
              ...
              if (mem[r + 6].b32.s1 == 0)
                ...
                BOX_glue_set(r) =
                    (t - mem[r + 1].b32.s1) / ((double)mem[r + 6].b32.s1);
            } else {
              ...
              if (mem[r + 4].b32.s1 == 0)
                ...
                BOX_glue_set(r) =
                    (mem[r + 1].b32.s1 - t) / ((double)mem[r + 4].b32.s1);
            }
            mem[r + 1].b32.s1 = w;
            NODE_type(r) = HLIST_NODE;
          } else { /*840: */
            ...
            if (t == mem[r + 3].b32.s1) {
              ...
              if (mem[r + 6].b32.s1 == 0)
                ...
                BOX_glue_set(r) =
                    (t - mem[r + 3].b32.s1) / ((double)mem[r + 6].b32.s1);
            } else {
              ...
              if (mem[r + 4].b32.s1 == 0)
                ...
                BOX_glue_set(r) =
                    (mem[r + 3].b32.s1 - t) / ((double)mem[r + 4].b32.s1);
            }
            mem[r + 3].b32.s1 = w;
            NODE_type(r) = VLIST_NODE;
          }
...
int32_t vert_break(int32_t p, scaled_t h, scaled_t d) {
  ...
  while (true) {
    ...
    if ((NODE_type(p) < GLUE_NODE) || (NODE_type(p) > KERN_NODE))
      goto not_found;
  lab90: /*update_heights */ /*1011: */
    if (NODE_type(p) == KERN_NODE)
      q = p;
    else {

      q = mem[p + 1].b32.s0;
      active_width[2 + mem[q].b16.s1] =
          active_width[2 + mem[q].b16.s1] + mem[q + 2].b32.s1;
...
void app_space(void) {
  ...
  if ((cur_list.aux.b32.s0 >= 2000) && (GLUEPAR(xspace_skip) != 0))
    ...
    if (cur_list.aux.b32.s0 >= 2000)
      mem[main_p + 1].b32.s1 =
          mem[main_p + 1].b32.s1 +
          font_info[EXTRA_SPACE_CODE + param_base[eqtb[CUR_FONT_LOC].b32.s1]]
              .b32.s1;
    mem[main_p + 2].b32.s1 =
        xn_over_d(mem[main_p + 2].b32.s1, cur_list.aux.b32.s0, 1000);
    mem[main_p + 3].b32.s1 =
        xn_over_d(mem[main_p + 3].b32.s1, 1000, cur_list.aux.b32.s0) /*:1079 */;
    q = new_glue(main_p);
...
bool its_all_over(void) {
  ...
  if (privileged()) {
    if ((PAGE_HEAD == page_tail) && (cur_list.head == cur_list.tail) &&
        (dead_cycles == 0)) {
      return true;
    }
    back_input();
...
void begin_box(int32_t box_context) {
  ...
  case LAST_BOX_CODE:
    ...
    if (abs(cur_list.mode) == MMODE) {
      ...
      if (tx < hi_mem_min) {
        if (NODE_type(tx) == HLIST_NODE ||
            NODE_type(tx) == VLIST_NODE) { /*1116:*/
          q = cur_list.head;
          p = TEX_NULL;
...
void package(small_number c) {
  ...
  if (cur_list.mode == -104)
    cur_box = hpack(mem[cur_list.head].b32.s1, save_stack[save_ptr + 2].b32.s1,
                    save_stack[save_ptr + 1].b32.s1);
  else {

    cur_box =
        vpackage(mem[cur_list.head].b32.s1, save_stack[save_ptr + 2].b32.s1,
                 save_stack[save_ptr + 1].b32.s1, d);
    if (c == VTOP_CODE) { /*1122: */
      h = 0;
      ...
      mem[cur_box + 2].b32.s1 =
          mem[cur_box + 2].b32.s1 - h + mem[cur_box + 3].b32.s1;
...
void new_graf(bool indented) {
  ...
  cur_list.prev_graf = (norm_min(INTPAR(left_hyphen_min)) * 64 +
                        norm_min(INTPAR(right_hyphen_min))) *
                           65536L +
                       cur_lang;
  if (indented) {
  ...
  if (cur_list.tail != cur_list.head) {
    if ((is_char_node(cur_list.tail)))
      ...
      if ((mem[cur_list.tail].b16.s0 == NATIVE_WORD_NODE) ||
        ...
        {
          mem[cur_list.tail].b32.s1 =
              new_kern(get_native_italic_correction(cur_list.tail));
          cur_list.tail = LLIST_link(cur_list.tail);
        }
    ...
    f = CHAR_NODE_font(p);
    {
      mem[cur_list.tail].b32.s1 = new_kern(FONT_CHARINFO_ITALCORR(
          f, FONT_CHARACTER_INFO(
                 f, effective_char(true, f, CHAR_NODE_character(p)))));
      cur_list.tail = LLIST_link(cur_list.tail);
    }
    NODE_subtype(cur_list.tail) = EXPLICIT;
  }
    ...
    if (c >= 0) {
      ...
      if (c <= BIGGEST_CHAR)
        mem[cur_list.tail + 1].b32.s0 =
            new_character(eqtb[CUR_FONT_LOC].b32.s1, c);
    }
  } else {
  ...
  if (p != TEX_NULL) {
    ...
    if (q != TEX_NULL) { /*1160: */
      ...
      if (((font_area[f] == AAT_FONT_FLAG) ||
           (font_area[f] == OTGR_FONT_FLAG))) {
        w = mem[q + 1].b32.s1;
        get_native_char_height_depth(f, cur_val, &h, &delta);
      } else {

        i = FONT_CHARACTER_INFO(
            f, effective_char(true, f, CHAR_NODE_character(q)));
...
void trap_zero_glue(void) {
  ...
  if ((mem[cur_val + 1].b32.s1 == 0) && (mem[cur_val + 2].b32.s1 == 0) &&
      (mem[cur_val + 3].b32.s1 == 0)) {
    GLUE_SPEC_ref_count(0)++;
    delete_glue_ref(cur_val);
    cur_val = 0;
  }
...
found:
  ...
  if (q < MULTIPLY) { /*1273:*/
    ...
    if (p < GLUE_VAL) {
      ...
      if (q == MULTIPLY) {
        mem[r + 1].b32.s1 =
            mult_and_add(mem[s + 1].b32.s1, cur_val, 0, MAX_HALFWORD);
        mem[r + 2].b32.s1 =
            mult_and_add(mem[s + 2].b32.s1, cur_val, 0, MAX_HALFWORD);
        mem[r + 3].b32.s1 =
            mult_and_add(mem[s + 3].b32.s1, cur_val, 0, MAX_HALFWORD);
      } else {
    ...
    if (f <= for_end)
      do {
        if (str_eq_str(font_name[f], cur_name) &&
            (((length(cur_area) == 0) && ((font_area[f] == AAT_FONT_FLAG) ||
                                          (font_area[f] == OTGR_FONT_FLAG))) ||
          ...
          if (s > 0) {
            if (s == font_size[f])
              goto common_ending;
                     ...
                     xn_over_d(font_dsize[f], -(int32_t)s, 1000))
            goto common_ending;
        }
        append_str(cur_area);
        append_str(cur_name);
  ...
  case 43: {
    if (abs(cur_list.mode) == VMODE) {
      ...
      if (((font_area[eqtb[CUR_FONT_LOC].b32.s1] == AAT_FONT_FLAG) ||
        ...
        set_native_glyph_metrics(cur_list.tail,
                                 (INTPAR(xetex_use_glyph_metrics) > 0));
      } else
        not_native_font_error(EXTENSION, GLYPH_CODE, eqtb[CUR_FONT_LOC].b32.s1);
    }
  } break;
  case 44: {
    scan_and_pack_name();
    ...
    if (i == XETEX_INPUT_MODE_AUTO) {
      ...
      {
        help_ptr = 2;
        help_line[1] = "You can't use `auto' encoding here, only for "
...
void insert_src_special(void) {
  ...
  if (source_filename_stack[in_open] > 0 &&
      is_new_source(source_filename_stack[in_open], line)) {
    toklist = get_avail();
    p = toklist;
...
void handle_right_brace(void) {
  ...
  case VCENTER_GROUP:
    ...
    unsave();
    save_ptr = save_ptr - 2;
    p = vpackage(mem[cur_list.head].b32.s1, save_stack[save_ptr + 1].b32.s1,
                 save_stack[save_ptr + 0].b32.s1, MAX_HALFWORD);
    pop_nest();
  ...
  default: {
    ...
    case 224:
      if (cur_chr == 2) {
        ...
        scan_fifteen_bit_int();
        set_math_char(set_class(cur_val / 4096) +
                      set_family((cur_val % 4096) / 256) + (cur_val % 256));
      }
      break;
    case 276: {
      set_math_char(set_class(cur_chr / 4096) +
                    set_family((cur_chr % 4096) / 256) + (cur_chr % 256));
    } break;
    case 277:
  ...
  lab71: /*collect_native */
    ...
    if ((INTPAR(xetex_inter_char_tokens) > 0) &&
      ...
      if (prev_class == ((CHAR_CLASS_LIMIT - 1))) {
        ...
        find_sa_element(INTER_CHAR_VAL,
                        prev_class * CHAR_CLASS_LIMIT + space_class, false);
        if (cur_ptr != TEX_NULL) {
          if (cur_cmd != LETTER)
            cur_cmd = OTHER_CHAR;
          cur_tok = (cur_cmd * MAX_CHAR_VAL) + cur_chr;
    ...
    if ((cur_chr > 65535L)) {
      while (native_text_size <= native_len + 2) {
        ...
        native_text =
            xrealloc(native_text, native_text_size * sizeof(UTF16_code));
      }
      {
          ...
          do {
            ...
            if ((main_h == 0) &&
                ((mapped_text[main_p] == hyphen_char[main_f]) ||
                 ((INTPAR(xetex_dash_break) > 0) &&
                  ((mapped_text[main_p] == 8212) ||
                   (mapped_text[main_p] == 8211)))))
              main_h = native_len;
    ...
    if (cur_list.mode == HMODE) {
      ...
      do {
        ...
        if ((((main_pp) != TEX_NULL && (!(is_char_node(main_pp))) &&
            ...
            (mem[main_pp + 4].b16.s2 == main_f) && (main_ppp != main_pp) &&
            (!(is_char_node(main_ppp))) && (NODE_type(main_ppp) != DISC_NODE)) {
          ...
          main_h = 0;
          while ((main_h < main_k) &&
                 (native_text[temp_ptr + main_h] != hyphen_char[main_f]) &&
                 ((!(INTPAR(xetex_dash_break) > 0)) ||
      ...
      if ((((main_pp) != TEX_NULL && (!(is_char_node(main_pp))) &&
          ...
          (!(is_char_node(main_ppp))) && (NODE_type(main_ppp) != DISC_NODE)) {
        mem[main_pp].b32.s1 =
            new_native_word_node(main_f, main_k + mem[main_pp + 4].b16.s1);
        cur_list.tail = mem[main_pp].b32.s1;
        {
          ...
          if (main_p <= for_end)
            do
              NATIVE_NODE_text(cur_list.tail)[main_p] =
                  NATIVE_NODE_text(main_pp)[main_p];
            while (main_p++ < for_end);
        }
        {
          register int32_t for_end;
          ...
          if (main_p <= for_end)
            do
              NATIVE_NODE_text(
                  cur_list.tail)[main_p + mem[main_pp + 4].b16.s1] =
                  native_text[main_p];
            while (main_p++ < for_end);
        }
        set_native_metrics(cur_list.tail,
      ...
      if ((main_pp != TEX_NULL)) {
        if (mem[main_pp + 4].b16.s2 == main_f) {
          ...
          while (!(is_char_node(main_p)) &&
                 ((NODE_type(main_p) == PENALTY_NODE) ||
                  (NODE_type(main_p) == INS_NODE) ||
                  (NODE_type(main_p) == MARK_NODE) ||
                  (NODE_type(main_p) == ADJUST_NODE) ||
                  ((NODE_type(main_p) == WHATSIT_NODE) &&
                   (mem[main_p].b16.s0 <= 4))))
            main_p = LLIST_link(main_p);
          if (!(is_char_node(main_p)) && (NODE_type(main_p) == GLUE_NODE)) {
            ...
            while (!(is_char_node(main_ppp)) &&
                   ((NODE_type(main_ppp) == PENALTY_NODE) ||
                    ...
                    ((NODE_type(main_ppp) == WHATSIT_NODE) &&
            ...
            if (main_ppp == cur_list.tail) {
              ...
              main_k++;
              {
                ...
                if (t <= for_end)
                  do {
                    NATIVE_NODE_text(temp_ptr)[main_k] =
                        NATIVE_NODE_text(cur_list.tail)[t];
                    main_k++;
                  } while (t++ < for_end);
              }
              set_native_metrics(temp_ptr,
                                 (INTPAR(xetex_use_glyph_metrics) > 0));
...
lab90: /*main_loop_move */ /*1071: */
  if (lig_stack == TEX_NULL)
    goto reswitch;
  cur_q = cur_list.tail;
  cur_l = mem[lig_stack].b16.s0;
lab91: /*main_loop_move 1 */
  if (!(is_char_node(lig_stack)))
    goto lab95;
lab92: /*main_loop_move 2 */
...
lab100: /*main_loop_lookahead */ /*1073: */
  get_next();
  if (cur_cmd == LETTER)
    goto lab101;
...
lab110: /*main_lig_loop */ /*1074: */
  ...
  if (main_j.s3 <= 128)
    goto lab112;
  main_k = lig_kern_base[main_f] + 256 * main_j.s1 + main_j.s0 + 32768L -
           256 * (128);
lab111: /*main_lig_loop 1 */
  main_j = font_info[main_k].b16;
lab112: /*main_lig_loop 2 */
  if (main_j.s2 == cur_r) {
    ...
    if (main_j.s3 <= 128) { /*1075: */
      if (main_j.s1 >= 128) {
        ...
        }
        {
          mem[cur_list.tail].b32.s1 = new_kern(
              font_info[kern_base[main_f] + 256 * main_j.s1 + main_j.s0]
                  .b32.s1);
          cur_list.tail = LLIST_link(cur_list.tail);
        }
        goto lab90;
      ...
      case 5: {
        cur_l = main_j.s0;
        main_i =
            FONT_CHARACTER_INFO(main_f, effective_char(true, main_f, cur_l));
...
void give_err_help(void) { token_show(LOCAL(err_help)); }
...
void scan_pdf_ext_toks(void) { scan_toks(false, true); }
