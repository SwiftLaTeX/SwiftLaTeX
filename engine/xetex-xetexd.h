...
#define xrealloc_array(ptr, type, size)                                        \
...
typedef struct b32x2_be_t {
  int32_t s1, s0;
} b32x2;
typedef struct b16x4_be_t {
  uint16_t s3, s2, s1, s0;
} b16x4;

#else

typedef struct b32x2_le_t {
  int32_t s0, s1;
} b32x2;
typedef struct b16x4_le_t {
 ...
 * - similar for `<quarterword_variable>.u.B<n>` => `<quarterword_variable>.s{{3
 * - <n>}}` !!!
 * - `w.hh.u.B0` => `w.b16.s1`
 * - `w.hh.u.B1` => `w.b16.s0`
 * - `w.hh.v.RH` => `w.b32.s1`
...
/* Symbolic accessors for various TeX data structures. I would loooove to turn
 * these into actual structs, but the path to doing that is not currently clear.
 * Making field references symbolic seems like a decent start. Sadly I don't see
 * how to do this conversion besides painstakingly annotating things.
 */
...
#define BOX_width(p)                                                           \
  mem[(p) + 1].b32.s1 /* a scaled; 1 <=> WEB const `width_offset` */
#define BOX_depth(p)                                                           \
  mem[(p) + 2].b32.s1 /* a scaled; 2 <=> WEB const `depth_offset` */
#define BOX_height(p)                                                          \
  mem[(p) + 3].b32.s1 /* a scaled; 3 <=> WEB const `height_offset` */
...
#define ACTIVE_NODE_break_node(p)                                              \
  mem[(p) + 1].b32.s1 /* aka "rlink" in double-linked list */
#define ACTIVE_NODE_line_number(p)                                             \
  mem[(p) + 1].b32.s0 /* aka "llink" in doubly-linked list */
#define ACTIVE_NODE_total_demerits(p)                                          \
  mem[(p) + 2].b32.s1 /* was originally the `mem[x+2].int` field */
#define ACTIVE_NODE_shortfall(p)                                               \
  mem[(p) + 3].b32.s1 /* a scaled; "active_short" in the WEB */
...
#define DELTA_NODE_dwidth(p)                                                   \
  mem[(p) + 1].b32.s1 /* the "natural width" difference */
#define DELTA_NODE_dstretch0(p)                                                \
  mem[(p) + 2].b32.s1 /* the stretch difference in points */
#define DELTA_NODE_dstretch1(p)                                                \
  mem[(p) + 3].b32.s1 /* the stretch difference in fil */
#define DELTA_NODE_dstretch2(p)                                                \
  mem[(p) + 4].b32.s1 /* the stretch difference in fill */
#define DELTA_NODE_dstretch3(p)                                                \
  mem[(p) + 5].b32.s1 /* the stretch difference in fill */
#define DELTA_NODE_dshrink(p) mem[(p) + 6].b32.s1 /* the shrink difference */

#define DISCRETIONARY_NODE_replace_count(p)                                    \
  mem[p].b16.s0 /* aka "subtype" of a node */
#define DISCRETIONARY_NODE_pre_break(p)                                        \
  mem[(p) + 1].b32.s0 /* aka "llink" in doubly-linked list */
#define DISCRETIONARY_NODE_post_break(p)                                       \
  mem[(p) + 1].b32.s1 /* aka "rlink" in double-linked list */

#define EDGE_NODE_edge_dist(p)                                                 \
  mem[(p) + 2].b32.s1 /* "new left_edge position relative to cur_h" */

#define GLUE_NODE_glue_ptr(p)                                                  \
  mem[(p) + 1].b32.s0 /* aka "llink" in doubly-linked list */
#define GLUE_NODE_leader_ptr(p)                                                \
  mem[(p) + 1].b32.s1 /* aka "rlink" in double-linked list */

#define INSERTION_NODE_float_cost(p)                                           \
  mem[(p) + 1].b32.s1 /* "the floating_penalty to be used" */
...
#define LANGUAGE_NODE_what_lang(p)                                             \
  mem[(p) + 1].b32.s1 /* language number, 0..255 */
#define LANGUAGE_NODE_what_lhm(p)                                              \
  mem[(p) + 1].b16.s1 /* "minimum left fragment, range 1..63" */
#define LANGUAGE_NODE_what_rhm(p)                                              \
  mem[(p) + 1].b16.s0 /* "minimum right fragment, range 1..63" */

#define LIGATURE_NODE_lig_font(p)                                              \
  mem[(p) + 1].b16.s1 /* WEB: font(lig_char(p)) */
#define LIGATURE_NODE_lig_char(p)                                              \
  mem[(p) + 1].b16.s0 /* WEB: character(lig_char(p)) */
#define LIGATURE_NODE_lig_ptr(p)                                               \
  mem[(p) + 1].b32.s1 /* WEB: link(lig_char(p)) */

#define MARK_NODE_ptr(p)                                                       \
  mem[(p) + 1].b32.s1 /* "head of the token list for the mark" */
...
#define MATH_NODE_end_lr_type(p)                                               \
  (L_CODE * (NODE_subtype(p) / L_CODE) + END_M_CODE)

#define NATIVE_NODE_size(p) mem[(p) + 4].b16.s3
#define NATIVE_NODE_font(p) mem[(p) + 4].b16.s2
#define NATIVE_NODE_length(p)                                                  \
  mem[(p) + 4].b16.s1 /* number of UTF16 items in the text */
#define NATIVE_NODE_glyph(p)                                                   \
  mem[(p) + 4].b16.s1 /* ... or the glyph number, if subtype==GLYPH_NODE */
...
#define PAGE_INS_NODE_broken_ptr(p)                                            \
  mem[(p) + 1]                                                                 \
      .b32.s1 /* "an insertion for this class will break here if anywhere" */
#define PAGE_INS_NODE_broken_ins(p)                                            \
  mem[(p) + 1].b32.s0 /* "this insertion might break at broken_ptr" */
#define PAGE_INS_NODE_last_ins_ptr(p)                                          \
  mem[(p) + 2].b32.s1 /* "the most recent insertion for this subtype" */
#define PAGE_INS_NODE_best_ins_ptr(p)                                          \
  mem[(p) + 2].b32.s0 /* "the optimum most recent insertion" */

#define PASSIVE_NODE_prev_break(p)                                             \
  mem[(p) + 1].b32.s0 /* aka "llink" in doubly-linked list */
#define PASSIVE_NODE_next_break(p) PASSIVE_NODE_prev_break(p) /* siggggghhhhh  \
                                                               */
#define PASSIVE_NODE_cur_break(p)                                              \
  mem[(p) + 1].b32.s1 /* aka "rlink" in double-linked list */
#define PASSIVE_NODE_serial(p) mem[p].b32.s0 /* aka "info" */

#define PENALTY_NODE_penalty(p)                                                \
  mem[(p) + 1].b32.s1 /* was originally the `mem[x+1].int` field */

#define PIC_NODE_path_len(p)                                                   \
  mem[(p) + 4].b16.s1 /* number of bytes in the path item */
#define PIC_NODE_path(p) ((unsigned char *)&mem[(p) + PIC_NODE_SIZE])
#define PIC_NODE_total_size(p)                                                 \
  (PIC_NODE_SIZE +                                                             \
   (PIC_NODE_path_len(p) + sizeof(memory_word) - 1) / sizeof(memory_word))

#define WRITE_NODE_tokens(p)                                                   \
  mem[(p) + 1].b32.s1 /* "reference count of token list to write" */
...
#define SYNCTEX_line(p, nodesize)                                              \
  mem[(p) + nodesize - SYNCTEX_FIELD_SIZE].b32.s1

#define GLUE_SPEC_ref_count(p)                                                 \
  mem[p].b32.s1 /* aka "link" of a link-list node */
...
#define FONT_CHARINFO_HEIGHT(f, info)                                          \
  font_info[height_base[f] + (info).s2 / 16].b32.s1
#define FONT_CHARINFO_DEPTH(f, info)                                           \
  font_info[depth_base[f] + (info).s2 % 16].b32.s1
#define FONT_CHARINFO_ITALCORR(f, info)                                        \
...
#define ETEX_MARK_sa_split_first_mark(p)                                       \
  mem[(p) + 2].b32.s1 /* \splitfirstmarks<n> */
#define ETEX_MARK_sa_split_bot_mark(p)                                         \
  mem[(p) + 3].b32.s0 /* \splitbotmarks<n> */
...
typedef struct {
  ...
  int32_t prev_graf; /* number of lines that have already been put into the
                        current vlist */
...
typedef struct {
  ...
  int32_t name;   /* string number: name of current file or magic value for
                     terminal, etc. */
  int32_t synctex_tag;
} input_state_t;
...
extern scaled_t cur_h,
    cur_v; /* should be internal to shipout, but accessed by synctex */
...
static inline bool is_char_node(const int32_t p) { return p >= hi_mem_min; }
...
static inline pool_pointer cur_length(void) {
...
#define get_native_italic_correction(p)                                        \
  real_get_native_italic_correction(&(mem[p]))
#define get_native_glyph_italic_correction(p)                                  \
  real_get_native_glyph_italic_correction(&(mem[p]))
...
#define AUTO                                                                   \
  0 /* default: will become one of 1..3 at file open time, after sniffing */
#define UTF8 1
#define UTF16BE 2
#define UTF16LE 3
#define RAW 4
