...
static void swap_items(char *p, size_t nitems, size_t size) {
  char temp;
...
static void do_dump(char *p, size_t item_size, size_t nitems,
                    rust_output_handle_t out_file) {
...
static void do_undump(char *p, size_t item_size, size_t nitems,
                      rust_input_handle_t in_file) {
...
static void primitive(const char *ident, uint16_t c, int32_t o) {
  int32_t prim_val;
...
void prefixed_command(void) {
  ...
  while (cur_cmd == PREFIX) {
    ...
    if (cur_cmd <= MAX_NON_PREFIXED_COMMAND) { /*1247:*/
      ...
      print_char('\'');
      help_ptr = 1;
      help_line[0] = "I'll pretend you didn't say \\long or \\outer or "
                     "\\global or \\protected.";
      back_error();
      return;
    }
  ...
  case SHORTHAND_DEF:
    if (cur_chr == CHAR_SUB_DEF_CODE) {
      ...
      if ((p - CHAR_SUB_CODE_BASE) < INTPAR(char_sub_def_min)) {
        if (a >= 4)
          geq_word_define(INT_BASE + INT_PAR__char_sub_def_min,
                          p - CHAR_SUB_CODE_BASE);
        else
          eq_word_define(INT_BASE + INT_PAR__char_sub_def_min,
                         p - CHAR_SUB_CODE_BASE);
      }

      if ((p - CHAR_SUB_CODE_BASE) > INTPAR(char_sub_def_max)) {
        if (a >= 4)
          geq_word_define(INT_BASE + INT_PAR__char_sub_def_max,
                          p - CHAR_SUB_CODE_BASE);
        else
          eq_word_define(INT_BASE + INT_PAR__char_sub_def_max,
                         p - CHAR_SUB_CODE_BASE);
      }
    } else {
      n = cur_chr;
  ...
  case DEF_CODE:
    ...
    if (p < MATH_CODE_BASE) {
      ...
      if (cur_val == 32768L)
        ...
        cur_val = set_class(cur_val / 4096) +
...
static void pack_buffered_name(small_number n, int32_t a, int32_t b) {
  free(name_of_file);
...
static bool load_fmt_file(void) {
  ...
  if (x != FORMAT_SERIAL)
    _tt_abort(
        "format file \"%s\" is of the wrong version: expected %d, found %d",
  ...
  do {
    ...
    p = q + mem[q].b32.s0;
    if (p > lo_mem_max ||
        (q >= mem[q + 1].b32.s1 && mem[q + 1].b32.s1 != rover))
      goto bad_fmt;
    q = mem[q + 1].b32.s1;
  } while (q != rover);
   ...
   * $(n, x_1, \ldots, x_n, m)$ in the format file represents $n + m$
  ...
  undump_checked_things(MIN_HALFWORD, MAX_HALFWORD, font_params[FONT_BASE],
                        font_ptr + 1);
  ...
  undump_checked_things(MIN_HALFWORD, lo_mem_max, font_glue[FONT_BASE],
                        font_ptr + 1);
...
static void initialize_more_variables(void) {
  int32_t k;
  hyph_pointer z;
...
static void initialize_more_initex_variables(void) {
  int32_t i, k;
  ...
  for (k = LOCAL_BASE + LOCAL__output_routine; k <= TOKS_BASE + NUMBER_REGS - 1;
       k++)
    eqtb[k] = eqtb[UNDEFINED_CONTROL_SEQUENCE];
...
/*:1370*/ /*1371: */
static void initialize_primitives(void) {
  ...
  primitive("XeTeXlinebreakskip", ASSIGN_GLUE,
  ...
  primitive("XeTeXinterchartoks", ASSIGN_TOKS,
            LOCAL_BASE + LOCAL__xetex_inter_char);
  primitive("TectonicCodaTokens", ASSIGN_TOKS,
            LOCAL_BASE + LOCAL__TectonicCodaTokens);
  ...
  primitive("exhyphenpenalty", ASSIGN_INT,
            INT_BASE + INT_PAR__ex_hyphen_penalty);
  ...
  primitive("displaywidowpenalty", ASSIGN_INT,
            INT_BASE + INT_PAR__display_widow_penalty);
  ...
  primitive("tracingparagraphs", ASSIGN_INT,
            INT_BASE + INT_PAR__tracing_paragraphs);
  ...
  primitive("tracinglostchars", ASSIGN_INT,
            INT_BASE + INT_PAR__tracing_lost_chars);
  primitive("tracingcommands", ASSIGN_INT,
            INT_BASE + INT_PAR__tracing_commands);
  primitive("tracingrestores", ASSIGN_INT,
            INT_BASE + INT_PAR__tracing_restores);
  ...
  primitive("floatingpenalty", ASSIGN_INT,
            INT_BASE + INT_PAR__floating_penalty);
  ...
  primitive("defaulthyphenchar", ASSIGN_INT,
            INT_BASE + INT_PAR__default_hyphen_char);
  primitive("defaultskewchar", ASSIGN_INT,
            INT_BASE + INT_PAR__default_skew_char);
  ...
  primitive("errorcontextlines", ASSIGN_INT,
            INT_BASE + INT_PAR__error_context_lines);
  ...
  primitive("scriptscriptfont", DEF_FAMILY,
...
tt_history_t tt_run_engine(char *dump_name, char *input_file_name,
  ...
  if (in_initex_mode) {
    ...
    primitive("XeTeXlinebreaklocale", EXTENSION,
    ...
    primitive("XeTeXfindvariationbyname", LAST_ITEM,
    ...
    primitive("XeTeXfindfeaturebyname", LAST_ITEM,
              XETEX_FIND_FEATURE_BY_NAME_CODE);
    primitive("XeTeXisexclusivefeature", LAST_ITEM,
    ...
    primitive("XeTeXfindselectorbyname", LAST_ITEM,
              XETEX_FIND_SELECTOR_BY_NAME_CODE);
    primitive("XeTeXisdefaultselector", LAST_ITEM,
    ...
    primitive("XeTeXOTcountlanguages", LAST_ITEM,
    ...
    primitive("tracingassigns", ASSIGN_INT,
  ...
  if (in_initex_mode) {
    ...
    get_date_and_time(build_date, &(INTPAR(time)), &(INTPAR(day)),
                      &(INTPAR(month)), &(INTPAR(year)));
  }
