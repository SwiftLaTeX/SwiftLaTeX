...
static small_number reconstitute(small_number j, small_number n, int32_t bchar,
                                 int32_t hchar);
...
static inline UnicodeScalar get_native_usv(int32_t p, int32_t i) {
...
void line_break(bool d) {
  ...
  pack_begin_line =
      cur_list.mode_line; /* "this is for over/underfull box messages" */

  LLIST_link(TEMP_HEAD) = LLIST_link(cur_list.head);

  /* Remove trailing space or glue if present; add infinite penalty then
   * par_fill_skip */
  ...
  last_line_fill = LLIST_link(cur_list.tail) =
      new_param_glue(GLUE_PAR__par_fill_skip);
  ...
  if (GLUE_SPEC_shrink_order(GLUEPAR(left_skip)) != NORMAL &&
      GLUE_SPEC_shrink(GLUEPAR(left_skip)) != 0)
    GLUEPAR(left_skip) = finite_shrink(GLUEPAR(left_skip));

  if (GLUE_SPEC_shrink_order(GLUEPAR(right_skip)) != NORMAL &&
      GLUE_SPEC_shrink(GLUEPAR(right_skip)) != 0)
  ...
  /* 1631: "check for special treatment of last line of paragraph" (\lastlinefit
   * > 0) */
    ...
    while (cur_p != TEX_NULL && LLIST_link(ACTIVE_LIST) != LAST_ACTIVE) {
      ...
      case GLUE_NODE:
        if (auto_breaking) {
          if (is_char_node(prev_p))
            ...
            try_break(0, UNHYPHENATED);
          else if (NODE_type(prev_p) == KERN_NODE &&
                   NODE_subtype(prev_p) != EXPLICIT)
            try_break(0, UNHYPHENATED);
        }
        ...
        if (second_pass && auto_breaking) {
          ...
          if (s != TEX_NULL) {
            /*930: skip to node ha, or goto done1 if no hyphenation should be
             * attempted */
            while (true) {
              if (is_char_node(s)) {
                ...
                hf = CHAR_NODE_font(q);
              } else if (NODE_type(s) == KERN_NODE &&
                         NODE_subtype(s) == NORMAL) {
                goto _continue;
              } else if (NODE_type(s) == MATH_NODE &&
                         NODE_subtype(s) >= L_CODE) {
                goto _continue;
          ...
          done2:
            ...
            if (ha != TEX_NULL && ha < hi_mem_min &&
                NODE_type(ha) == WHATSIT_NODE &&
                (NODE_subtype(ha) == NATIVE_WORD_NODE ||
                 NODE_subtype(ha) == NATIVE_WORD_NODE_AT)) {
              /*926: check that nodes after native_word permit hyphenation; if
               * not, goto done1 */
              s = LLIST_link(ha);
            ...
            restart:
              ...
              while (true) {
                if (is_char_node(s)) {
                  ...
                  /*932: move the characters of a ligature node to hu and hc;
      ...
      case LIGATURE_NODE:
        ...
        active_width[1] += FONT_CHARACTER_WIDTH(
            f, effective_char(true, f, LIGATURE_NODE_lig_char(cur_p)));
        break;
            ...
            if (is_char_node(s)) {
              ...
              case WHATSIT_NODE:
                if (NODE_subtype(s) == NATIVE_WORD_NODE ||
                    ...
                    NODE_subtype(s) == PIC_NODE || NODE_subtype(s) == PDF_NODE)
                  disc_width += BOX_width(s);
                else
                  confusion("disc3a");
                break;
          ...
          if (NODE_type(r) != DELTA_NODE) {
            ...
            if ((line_diff < actual_looseness &&
              ...
              fewest_demerits = ACTIVE_NODE_total_demerits(r);
                       ...
                       ACTIVE_NODE_total_demerits(r) < fewest_demerits) {
              best_bet = r;
              fewest_demerits = ACTIVE_NODE_total_demerits(r);
            }
          }
  ...
  if (do_last_line_fit) { /*1641:*/
    if (ACTIVE_NODE_shortfall(best_bet) == 0) {
      ...
      BOX_width(q) +=
          ACTIVE_NODE_shortfall(best_bet) - ACTIVE_NODE_glue(best_bet);
...
static void post_line_break(bool d) {
  ...
  do {
    ...
    if (INTPAR(texxet) > 0) { /*1494:*/
      ...
      while (q != mem[cur_p + 1].b32.s1) {
        if (q < hi_mem_min && NODE_type(q) == MATH_NODE) { /*1495:*/
          if (odd(mem[q].b16.s0)) {
            if (LR_ptr != TEX_NULL &&
                mem[LR_ptr].b32.s0 == (L_CODE * (mem[q].b16.s0 / L_CODE) + 3)) {
              temp_ptr = LR_ptr;
    ...
    /* "at this point q is the leftmost node; all discardable nodes have been
     * discarded */
      ...
      if (!post_disc_break) {
        ...
        while (true) {
          ...
          if (is_non_discardable_node(q))
            break;
          if (NODE_type(q) == KERN_NODE && NODE_subtype(q) != EXPLICIT &&
...
static void try_break(int32_t pi, small_number break_type) {
    ...
    if (l > old_l) { /* "now we are no longer in the inner loop" */
      if (minimum_demerits < AWFUL_BAD &&
        ...
        if (no_break_yet) {
              ...
              while (t > 0) {
                ...
                if (is_char_node(v)) {
                  ...
                  case LIGATURE_NODE: {
                    ...
                    xtx_ligature_present = true;
                    eff_char =
                        effective_char(true, f, LIGATURE_NODE_lig_char(v));
                    break_width[1] -= FONT_CHARACTER_WIDTH(f, eff_char);
                    break;
                  }
          ...
          while (s != TEX_NULL) {
            ...
            case GLUE_NODE:
              ...
              break_width[1] -= BOX_width(v);
              break_width[2 + GLUE_SPEC_stretch_order(v)] -=
                  GLUE_SPEC_stretch(v);
        ...
        for (fit_class = VERY_LOOSE_FIT; fit_class <= TIGHT_FIT; fit_class++) {
          if (minimal_demerits[fit_class] <= minimum_demerits) {
            /*874: "Insert a new active node from best_place[fit_class] to
             * cur_p" */
      ...
      if (l > easy_line) {
        ...
        if (l > last_special_line)
          ...
          line_width = mem[LOCAL(par_shape) + 2 * l]
                           .b32.s1; /* this mem access is in the WEB */
      }
    }
      ...
      if (cur_active_width[3] != 0 || cur_active_width[4] != 0 ||
        ...
        if (do_last_line_fit) {
          if (cur_p == TEX_NULL) {
            ...
            arith_error = false;
            g = fract(g, ACTIVE_NODE_shortfall(r), ACTIVE_NODE_glue(r),
                      MAX_HALFWORD);
            ...
            if (g > 0) {
              ...
              if (g > 7230584L) { /* XXX: magic number in original WEB code */
                if (cur_active_width[2] <
                    1663497L) { /* XXX: magic number in original WEB code */
                  b = INF_BAD;
  ...
  found:
    if (b > INF_BAD || pi == EJECT_PENALTY) {
      ...
      if (final_pass && minimum_demerits == AWFUL_BAD &&
          LLIST_link(r) == LAST_ACTIVE && prev_r == ACTIVE_LIST)
        artificial_demerits = true;
      else if (b > threshold)
        goto deactivate;
  ...
  deactivate:
    ...
    if (prev_r == ACTIVE_LIST) {
      /*890: "Update the active widths, since the first active node has been
       * deleted" */
      r = LLIST_link(ACTIVE_LIST);
...
found1:
  if ((((ha) != TEX_NULL && (!(is_char_node(ha))) &&
        (NODE_type(ha) == WHATSIT_NODE) &&
        ((mem[ha].b16.s0 == NATIVE_WORD_NODE) ||
         (mem[ha].b16.s0 == NATIVE_WORD_NODE_AT))))) {
    s = cur_p;
        ...
        do {
          if (odd(hyf[j])) {
            ...
            mem[q].b16.s0 = mem[ha].b16.s0;
            {
              ...
              if (i <= for_end)
                do
                  NATIVE_NODE_text(q)[i] =
                      NATIVE_NODE_text(ha)[i + hyphen_passed];
                while (i++ < for_end);
            }
            set_native_metrics(q, (INTPAR(xetex_use_glyph_metrics) > 0));
            mem[s].b32.s1 = q;
...
static scaled_t total_pw(int32_t q, int32_t p) {
  ...
  if ((p != TEX_NULL) && (NODE_type(p) == DISC_NODE) &&
...
static int32_t find_protchar_left(int32_t l, bool d) {
  int32_t t;
  bool run;
  if ((mem[l].b32.s1 != TEX_NULL) && (NODE_type(l) == HLIST_NODE) &&
      (mem[l + 1].b32.s1 == 0) && (mem[l + 3].b32.s1 == 0) &&
      (mem[l + 2].b32.s1 == 0) && (mem[l + 5].b32.s1 == TEX_NULL))
    l = LLIST_link(l);
  else if (d)
    while ((mem[l].b32.s1 != TEX_NULL) &&
  ...
  do {
    t = l;
    while (run && (NODE_type(l) == HLIST_NODE) &&
           (mem[l + 5].b32.s1 != TEX_NULL)) {
    ...
    while (run &&
           (!(is_char_node(l)) &&
            ((NODE_type(l) == INS_NODE) || (NODE_type(l) == MARK_NODE) ||
             ...
             ((NODE_type(l) == HLIST_NODE) && (mem[l + 1].b32.s1 == 0) &&
              (mem[l + 3].b32.s1 == 0) && (mem[l + 2].b32.s1 == 0) &&
              (mem[l + 5].b32.s1 == TEX_NULL))))) {
...
static int32_t find_protchar_right(int32_t l, int32_t r) {
  ...
  do {
    t = r;
    while (run && (NODE_type(r) == HLIST_NODE) &&
    ...
    while (run &&
           (!(is_char_node(r)) &&
            ((NODE_type(r) == INS_NODE) || (NODE_type(r) == MARK_NODE) ||
             ...
             ((NODE_type(r) == HLIST_NODE) && (mem[r + 1].b32.s1 == 0) &&
              (mem[r + 3].b32.s1 == 0) && (mem[r + 2].b32.s1 == 0) &&
              (mem[r + 5].b32.s1 == TEX_NULL))))) {
