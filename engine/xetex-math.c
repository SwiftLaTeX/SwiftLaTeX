...
static int32_t build_opentype_assembly(internal_font_number f, void *a,
                                       scaled_t s, bool horiz);
...
static small_number make_left_right(int32_t q, small_number style,
                                    scaled_t max_d, scaled_t max_h);
...
void init_math(void) {
  ...
  if ((cur_cmd == MATH_SHIFT) && (cur_list.mode > 0)) { /*1180: */
    ...
    if (cur_list.head == cur_list.tail) { /*1520: */
      ...
      v = v + 2 * font_info[QUAD_CODE + param_base[eqtb[CUR_FONT_LOC].b32.s1]]
                      .b32.s1;
      ...
      reswitch:
        ...
        case 9: {
          ...
          if (INTPAR(texxet) > 0) { /*1525: */
            if (odd(mem[p].b16.s0)) {
              if (mem[LR_ptr].b32.s0 ==
                  (L_CODE * (mem[p].b16.s0 / L_CODE) + 3)) {
                temp_ptr = LR_ptr;
        ...
        case 10: {
          ...
          if (mem[just_box + 5].b16.s1 == STRETCHING) {
            if ((mem[just_box + 5].b16.s0 == mem[q].b16.s1) &&
                (mem[q + 2].b32.s1 != 0))
              v = MAX_HALFWORD;
          } else if (mem[just_box + 5].b16.s1 == SHRINKING) {
            if ((mem[just_box + 5].b16.s0 == mem[q].b16.s0) &&
                (mem[q + 3].b32.s1 != 0))
              v = MAX_HALFWORD;
          }
        ...
        case 8:
          if ((mem[p].b16.s0 == NATIVE_WORD_NODE) ||
...
static void scan_delimiter(int32_t p, bool r) {
  if (r) {
    if (cur_chr == 1) {
      cur_val1 = 0x40000000;
      scan_math_fam_int();
  ...
  if (cur_val >= 0x40000000) {
    mem[p].b16.s3 =
        ((cur_val % 0x200000) / 0x10000) * 0x100 + (cur_val / 0x200000) % 0x100;
...
void math_ac(void) {
  ...
  if (cur_chr == 1) {
    ...
    cur_val = set_class(cur_val / 4096) + set_family((cur_val % 4096) / 256) +
              (cur_val % 256);
  }
  ...
  mem[cur_list.tail + 4].b16.s1 =
      mem[cur_list.tail + 4].b16.s1 + (math_char(cur_val) / 65536L) * 256;
  scan_math(cur_list.tail + 1);
}
...
static void app_display(int32_t j, int32_t b, scaled_t d) {
  scaled_t z;
  scaled_t s;
  scaled_t e;
  int32_t x;
  ...
  if (((font_params[MATH_FONT(2)] < TOTAL_MATHSY_PARAMS) &&
    ...
    danger = true;
              ...
              (!((font_area[MATH_FONT(3 + TEXT_SIZE)] == OTGR_FONT_FLAG) &&
                 (isOpenTypeMathFont(
                     font_layout_engine[MATH_FONT(3 + TEXT_SIZE)]))))) ||
             ...
             ((font_params[MATH_FONT(3 + SCRIPT_SCRIPT_SIZE)] <
               TOTAL_MATHEX_PARAMS) &&
              (!((font_area[MATH_FONT(3 + SCRIPT_SCRIPT_SIZE)] ==
                  OTGR_FONT_FLAG) &&
                 (isOpenTypeMathFont(font_layout_engine[MATH_FONT(
                     3 + SCRIPT_SCRIPT_SIZE)])))))) {
    {
      if (file_line_error_style_p)
        print_file_line();
      else
    ...
    if (((font_params[MATH_FONT(2)] < TOTAL_MATHSY_PARAMS) &&
        ...
        ((font_params[MATH_FONT(2 + SCRIPT_SIZE)] < TOTAL_MATHSY_PARAMS) &&
         (!((font_area[MATH_FONT(2 + SCRIPT_SIZE)] == OTGR_FONT_FLAG) &&
            (isOpenTypeMathFont(
                font_layout_engine[MATH_FONT(2 + SCRIPT_SIZE)]))))) ||
        ((font_params[MATH_FONT(2 + SCRIPT_SCRIPT_SIZE)] <
          TOTAL_MATHSY_PARAMS) &&
         (!((font_area[MATH_FONT(2 + SCRIPT_SCRIPT_SIZE)] == OTGR_FONT_FLAG) &&
            (isOpenTypeMathFont(
                font_layout_engine[MATH_FONT(2 + SCRIPT_SCRIPT_SIZE)])))))) {
      ...
      danger = true;
                ...
                (!((font_area[MATH_FONT(3 + TEXT_SIZE)] == OTGR_FONT_FLAG) &&
                   (isOpenTypeMathFont(
                       font_layout_engine[MATH_FONT(3 + TEXT_SIZE)]))))) ||
               ((font_params[MATH_FONT(3 + SCRIPT_SIZE)] <
                 TOTAL_MATHEX_PARAMS) &&
                (!((font_area[MATH_FONT(3 + SCRIPT_SIZE)] == OTGR_FONT_FLAG) &&
                   (isOpenTypeMathFont(
                       font_layout_engine[MATH_FONT(3 + SCRIPT_SIZE)]))))) ||
...
static scaled_t math_x_height(int32_t size_code) {
  ...
  if (((font_area[f] == OTGR_FONT_FLAG) &&
       (isOpenTypeMathFont(font_layout_engine[f]))))
...
static int32_t math_glue(int32_t g, scaled_t m) {
  ...
  mem[p + 1].b32.s1 =
      mult_and_add(n, mem[g + 1].b32.s1,
                   xn_over_d(mem[g + 1].b32.s1, f, 65536L), MAX_HALFWORD);
  mem[p].b16.s1 = mem[g].b16.s1;
  if (mem[p].b16.s1 == NORMAL)
    mem[p + 2].b32.s1 =
        mult_and_add(n, mem[g + 2].b32.s1,
                     xn_over_d(mem[g + 2].b32.s1, f, 65536L), MAX_HALFWORD);
  else
    mem[p + 2].b32.s1 = mem[g + 2].b32.s1;
  mem[p].b16.s0 = mem[g].b16.s0;
  if (GLUE_SPEC_shrink_order(p) == NORMAL)
    mem[p + 3].b32.s1 =
        mult_and_add(n, mem[g + 3].b32.s1,
                     xn_over_d(mem[g + 3].b32.s1, f, 65536L), MAX_HALFWORD);
  else
...
static void math_kern(int32_t p, scaled_t m) {
  ...
  if (mem[p].b16.s0 == MU_GLUE) {
    ...
    mem[p + 1].b32.s1 =
        mult_and_add(n, mem[p + 1].b32.s1,
                     xn_over_d(mem[p + 1].b32.s1, f, 65536L), MAX_HALFWORD);
...
found:
  ...
  else if ((mem[q].b32.s1 == TEX_NULL) && (NODE_type(q) <= VLIST_NODE) &&
           (mem[q + 4].b32.s1 == 0))
    x = q;
  else
    x = hpack(q, 0, ADDITIONAL);
...
static void fetch(int32_t a) {
  ...
  if (cur_f == FONT_BASE) { /*749: */
    ...
    mem[a].b32.s1 = EMPTY;
  } else if (((font_area[cur_f] == AAT_FONT_FLAG) ||
...
static void make_over(int32_t q) {
  mem[q + 1].b32.s0 =
      overbar(clean_box(q + 1, 2 * (cur_style / 2) + 1),
...
static void make_radical(int32_t q) {
  ...
  y = var_delimiter(q + 4, cur_size,
                    mem[x + 3].b32.s1 + mem[x + 2].b32.s1 + clr +
                        rule_thickness);
  if (((font_area[f] == OTGR_FONT_FLAG) &&
       (isOpenTypeMathFont(font_layout_engine[f])))) {
...
static void make_math_accent(int32_t q) {
  ...
  if (((font_area[cur_f] == AAT_FONT_FLAG) ||
    ...
    if (mem[q + 1].b32.s1 == MATH_CHAR) {
      ...
      if (((cur_i.s1) % 4) == LIG_TAG) {
        ...
        if (cur_i.s3 > 128) {
          a = lig_kern_base[cur_f] + 256 * cur_i.s1 + cur_i.s0 + 32768L -
              256 * (128);
          cur_i = font_info[a].b16;
        }
        while (true) {
          ...
          if (cur_i.s2 == skew_char[cur_f]) {
            if (cur_i.s1 >= 128) {
              ...
              if (cur_i.s3 <= 128)
                s = font_info[kern_base[cur_f] + 256 * cur_i.s1 + cur_i.s0]
                        .b32.s1;
            }
            goto done1;
          }
  ...
  if (x != TEX_NULL) {
    if (((font_area[f] == OTGR_FONT_FLAG) &&
      ...
      if (((mem[q].b16.s0 == BOTTOM_ACC) ||
           (mem[q].b16.s0 == (BOTTOM_ACC + 1))))
        delta = 0;
...
static void make_fraction(int32_t q) {
  ...
  if (mem[q + 1].b32.s1 == 0) { /*772:*/
    ...
    delta = half(clr - ((shift_up - mem[x + 2].b32.s1) -
                        (mem[z + 3].b32.s1 - shift_down)));
    ...
    if (font_area[cur_f] == OTGR_FONT_FLAG &&
      ...
      delta1 = clr - ((shift_up - mem[x + 2].b32.s1) -
      ...
      delta2 = clr - ((axis_height(cur_size) - delta) -
                      (mem[z + 3].b32.s1 - shift_down));
    } else {
      if (cur_style < TEXT_STYLE)
  ...
  if (mem[q + 1].b32.s1 == 0) {
    p = new_kern((shift_up - mem[x + 2].b32.s1) -
                 (mem[z + 3].b32.s1 - shift_down));
    mem[p].b32.s1 = z;
  } else {
...
static scaled_t make_op(int32_t q) {
  ...
  if (mem[q + 1].b32.s1 == MATH_CHAR) {
    fetch(q + 1);
    if (!((font_area[cur_f] == OTGR_FONT_FLAG) &&
          (usingOpenType(font_layout_engine[cur_f])))) {
    ...
    mem[x + 4].b32.s1 =
        half(mem[x + 3].b32.s1 - mem[x + 2].b32.s1) - axis_height(cur_size);
    mem[q + 1].b32.s1 = SUB_BOX;
    mem[q + 1].b32.s0 = x;
  }
  save_f = cur_f;
  if (mem[q].b16.s0 == LIMITS) { /*777: */
    ...
    if (mem[q + 2].b32.s1 == EMPTY) {
      ...
      mem[p].b32.s1 = x;
      mem[v + 5].b32.s1 = p;
      mem[v + 3].b32.s1 = mem[v + 3].b32.s1 + big_op_spacing5() +
                          mem[x + 3].b32.s1 + mem[x + 2].b32.s1 + shift_up;
    }
    if (mem[q + 3].b32.s1 == EMPTY)
      ...
      p = new_kern(big_op_spacing5());
      mem[z].b32.s1 = p;
      mem[v + 2].b32.s1 = mem[v + 2].b32.s1 + big_op_spacing5() +
                          mem[z + 3].b32.s1 + mem[z + 2].b32.s1 + shift_down;
    }
    mem[q + 1].b32.s1 = v;
  }
  free_ot_assembly(ot_assembly_ptr);
            ...
            if (mem[p + 1].b32.s1 == MATH_CHAR) {
              ...
              if ((mem[p + 1].b16.s1 % 256) == (mem[q + 1].b16.s1 % 256)) {
                ...
                if (((cur_i.s1) % 4) == LIG_TAG) {
                  ...
                  cur_i = font_info[a].b16;
                  if (cur_i.s3 > 128) {
                    a = lig_kern_base[cur_f] + 256 * cur_i.s1 + cur_i.s0 +
                        32768L - 256 * (128);
                    cur_i = font_info[a].b16;
                  }
                  while (true) {
...
static void make_scripts(int32_t q, scaled_t delta) {
  ...
  if (mem[q + 2].b32.s1 == EMPTY) { /*784: */
    ...
    if (((font_area[cur_f] == OTGR_FONT_FLAG) &&
         (isOpenTypeMathFont(font_layout_engine[cur_f])))) { /*787: */
      ...
      if ((((p) != TEX_NULL && (!(is_char_node(p))) &&
            (NODE_type(p) == WHATSIT_NODE) && (mem[p].b16.s0 == GLYPH_NODE))))
        sub_kern = get_ot_math_kern(mem[p + 4].b16.s2, mem[p + 4].b16.s1,
                                    script_f, script_g, SUB_CMD, shift_down);
      if (sub_kern != 0)
...

    {
      ...
      if (((font_area[cur_f] == OTGR_FONT_FLAG) &&
           (isOpenTypeMathFont(font_layout_engine[cur_f]))))
        clr = mem[x + 2].b32.s1 +
              get_ot_math_constant(cur_f, SUPERSCRIPTBOTTOMMIN);
      else
        clr = mem[x + 2].b32.s1 + (abs(math_x_height(cur_size)) / 4);
      if (shift_up < clr)
        shift_up = clr;
      if (((font_area[cur_f] == OTGR_FONT_FLAG) &&
           (isOpenTypeMathFont(font_layout_engine[cur_f])))) { /*788: */
        ...
        if ((((p) != TEX_NULL && (!(is_char_node(p))) &&
              (NODE_type(p) == WHATSIT_NODE) && (mem[p].b16.s0 == GLYPH_NODE))))
          sup_kern = get_ot_math_kern(mem[p + 4].b16.s2, mem[p + 4].b16.s1,
                                      script_f, script_g, SUP_CMD, shift_up);
    ...
    if (mem[q + 3].b32.s1 == EMPTY)
      ...
      if (((font_area[cur_f] == OTGR_FONT_FLAG) &&
        ...
        clr = 4 * default_rule_thickness() - ((shift_up - mem[x + 2].b32.s1) -
      ...
      if (((font_area[cur_f] == OTGR_FONT_FLAG) &&
           (isOpenTypeMathFont(font_layout_engine[cur_f])))) {
        {
          ...
          if ((((p) != TEX_NULL && (!(is_char_node(p))) &&
                ...
                (mem[p].b16.s0 == GLYPH_NODE))))
            sub_kern =
                get_ot_math_kern(mem[p + 4].b16.s2, mem[p + 4].b16.s1, script_f,
                                 script_g, SUB_CMD, shift_down);
          if (sub_kern != 0)
            p = attach_hkern_to_new_hlist(q, sub_kern);
        }
        {
...
static void mlist_to_hlist(void) {
  ...
  while (q != TEX_NULL) {
    ...
    if (r_type > 0) {
      const char *offset_table[] = {"02340001", "22*40001", "33**3**3",
                                    "44*04004", "00*00000", "02340001",
                                    "11*11111", "12341011"};
    ...
    if ((z != 0) || (x != 0)) {
      ...
      do {
        ...
        if (g != FONT_BASE) { /*734: */
          ...
          if (((font_area[g] == OTGR_FONT_FLAG) &&
               (usingOpenType(font_layout_engine[g])))) {
...
found:
  if (f != FONT_BASE) {
    if (!((font_area[f] == OTGR_FONT_FLAG) &&
          (usingOpenType(font_layout_engine[f])))) { /*736: */
      ...
      if (((q.s1) % 4) == EXT_TAG) { /*739: */
        ...
        mem[b + 1].b32.s1 =
            FONT_CHARINFO_WIDTH(f, q) + FONT_CHARINFO_ITALCORR(f, q);
        c = r.s1;
        if (c != 0)
  ...
  mem[b + 4].b32.s1 =
      half(mem[b + 3].b32.s1 - mem[b + 2].b32.s1) - axis_height(s);
  free_ot_assembly(ot_assembly_ptr);
  return b;
}
