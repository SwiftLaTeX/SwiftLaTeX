...
void initialize_pagebuilder_variables(void) { page_max_depth = 0; }
...
static void fire_up(int32_t c) {
  ...
  if (NODE_type(best_page_break) == PENALTY_NODE) {
    geq_word_define(INT_BASE + INT_PAR__output_penalty,
                    PENALTY_NODE_penalty(best_page_break));
  ...
  insert_penalties =
      0; /* "this will count the number of insertions held over" */
  ...
  process_inserts =
      (INTPAR(holding_inserts) <= 0) && !semantic_pagination_enabled;
  ...
  if (process_inserts) {
    ...
    while (r != PAGE_INS_HEAD) {
      if (PAGE_INS_NODE_best_ins_ptr(r) != TEX_NULL) {
        ...
        p = BOX_REG(n) +
            5; /* 5 = list_offset, "position of the list inside the box" */
        ...
        if (PAGE_INS_NODE_best_ins_ptr(r) == TEX_NULL) {
          ...
          if (PAGE_INS_NODE_best_ins_ptr(r) == p) {
            ...
            if (NODE_type(r) == SPLIT_UP) {
              if (PAGE_INS_NODE_broken_ins(r) == p &&
                  PAGE_INS_NODE_broken_ptr(r) != TEX_NULL) {
                while (LLIST_link(s) != PAGE_INS_NODE_broken_ptr(r))
                  s = LLIST_link(s);

                LLIST_link(s) = TEX_NULL;
                GLUEPAR(split_top_skip) = INSERTION_NODE_split_top_ptr(p);
                INSERTION_NODE_ins_ptr(p) =
                    prune_page_top(PAGE_INS_NODE_broken_ptr(r), false);

                if (INSERTION_NODE_ins_ptr(p) != TEX_NULL) {
                  temp_ptr = vpackage(INSERTION_NODE_ins_ptr(p), 0, ADDITIONAL,
                                      MAX_HALFWORD);
  ...
  BOX_REG(255) =
      vpackage(LLIST_link(PAGE_HEAD), best_size, EXACTLY, page_max_depth);
...
void build_page(void) {
  ...
  do {
    ...
    case RULE_NODE:
      if (page_contents < BOX_THERE) {
        ...
        q = new_skip_param(
            GLUE_PAR__top_skip); /* "now temp_ptr = glue_ptr(q) */
    ...
    case WHATSIT_NODE:
      /*1401: "Prepare to move whatsit p to the current page, then goto
       * contribute" */
    ...
    case INS_NODE:
      ...
      if (NODE_type(r) == SPLIT_UP) {
        ...
        if ((h <= 0 || h <= delta) &&
            BOX_height(p) + BOX_height(r) <= SCALED_REG(n)) {
          page_so_far[0] -= h;
    ...
    if (pi < INF_PENALTY) {
      ...
      if (c == AWFUL_BAD || pi <= EJECT_PENALTY) {
        ...
        if (output_active) /* "user's output routine will act" */
          return;
        goto done; /* "the page has been shipped out by the default output
                      routine" */
      }
    }
  ...
  update_heights:
    /*1039: "Update the current page measurements with respect to the glue or
