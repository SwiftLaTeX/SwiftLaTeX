...
LightLatexEditor::LightLatexEditor(QWidget *parent, QFont &efont,
                                   QList<QColor> edcolors,
                                   QList<QColor> hicolors, QString name)
...
bool LightLatexEditor::search(const QString &expr, bool cs, bool wo,
                              bool forward, bool startAtCursor, bool isRegExp) {
...
int LightLatexEditor::searchInSelection(const QString &expr, bool cs, bool wo,
...
void LightLatexEditor::changeFont(QFont &new_font) { setFont(new_font); }

QString LightLatexEditor::getEncoding() { return encoding; }

void LightLatexEditor::setEncoding(QString enc) { encoding = enc; }
...
int LightLatexEditor::numoflines() { return document()->blockCount(); }
...
bool LightLatexEditor::matchLeftPar(QTextBlock currentBlock, int index,
                                    int numLeftPar) {
...
bool LightLatexEditor::matchRightPar(QTextBlock currentBlock, int index,
                                     int numRightPar) {
...
int LightLatexEditor::getLastNumLines() { return lastnumlines; }

void LightLatexEditor::setLastNumLines(int n) { lastnumlines = n; }
...
void LightLatexEditor::load(const QString &f) {
  ...
  // hasDecodingError = (minSize < buf.size()- 4 || memcmp(verifyBuf.constData()
  // + verifyBuf.size() - minSize,buf.constData() + buf.size() - minSize,
  // minSize)); #endif QString new_encoding; QEncodingProber prober
  // (QEncodingProber::Universal); if (hasDecodingError)
  //   {
  //   prober.feed (buf.constData());
  //   QTextCodec* detected_codec;
  //   if (prober.confidence() > 0.5) //Kencodingprober works very bad with tex
  //   documents
  //     {
  ...
  //   else new_encoding=QString(QTextCodec::codecForLocale()->name());
  //   EncodingDialog *encDlg = new EncodingDialog(this);
  //   encDlg->ui.comboBoxEncoding->setCurrentIndex(encDlg->ui.comboBoxEncoding->findText(new_encoding,
  //   Qt::MatchExactly)); encDlg->ui.label->setText(encDlg->ui.label->text()+ "
  //   ("+encoding+")."); if (encDlg->exec())
  // 	  {
  // 	  new_encoding=encDlg->ui.comboBoxEncoding->currentText();
  // 	  codec = QTextCodec::codecForName(new_encoding.toLatin1());
  // 	  text = codec->toUnicode(buf);
...
void LightLatexEditor::editFind() { emit requestFind(); }

void LightLatexEditor::editGotoLine() { emit requestGotoLine(); }
