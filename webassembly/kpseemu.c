...
static int kpse_fetch_from_network(const char *name_ret, int format) {
  ...
  if (destfile) {
    curl = curl_easy_init();
    if (curl) {
      snprintf(name_url, MAXFILENAMESIZE, "%s/%s/%s", KPSE_BASE_URL,
               name_copy, format);
      ...
      if (curl_res == CURLE_OK) {
        ...
        if (response_code == 200) {
          ...
          char name_404[MAXFILENAMESIZE] = {0};
          snprintf(name_404, MAXFILENAMESIZE, "%s/%s.404", tmpfolder,
                   name_copy);
...
char *kpse_find_file(const_string name, kpse_file_format_type format,
  ...
  // If request file is tikz.sty or pgfcore.sty, we change it to dvitikz.sty when we are using nopdfout
  if (!nopdfoutput) {
    /* SwiftLaTeX */
    if(strncmp(local_name, "tikz.sty", 8) == 0 || strncmp(local_name, "pgfcore.sty", 11) == 0) {
      _prepend(local_name, "swiftlatex");
    }
  }
