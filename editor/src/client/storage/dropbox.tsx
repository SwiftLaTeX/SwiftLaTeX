...
export class DropboxStorage extends BackendStorage {
    ...
    async _getShareLink(itemKey: string): Promise<string | undefined> {
        const cacheAns = await this._getCache(itemKey);
        if (cacheAns) {
            return cacheAns;
        }
        ...
        await this._putCache(itemKey, rUrl);
        return rUrl;
    }
    ...
    async _putCache(itemKey: string, url: string) {
        if (url) {
            await set(itemKey + 'dropbox', url)
        }
    }

    async _getCache(itemKey: string): Promise<string | undefined> {
        const r = await get(itemKey + 'dropbox')
        if (!r) {
            return undefined;
        }
        return r as string;
