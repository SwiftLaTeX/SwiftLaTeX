...
unsigned long long getcharsigslot(int index, int line, int column, int character, int cs)
{
...
void outputfileinfo() {
    ...
    for(int i = 0; i < usedFileSlots; i ++) {
        pdf_printf("%s|", usedFilenameStack[i]);
    }
    pdf_printf(")\n");
}

void outputloc(int p, int style) {
    struct meta_struct *tmp = (struct meta_struct *)(&(zmem[p].hh.me));
    
    pdf_printf(")]TJ BX %d %d %d %d %d z EX [(", tmp->line, tmp->column, tmp->fid, tmp->cs, tmp->group);
}
