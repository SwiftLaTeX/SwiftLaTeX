...
UnicodeDialog::UnicodeDialog(QWidget *parent) : QDialog(parent) {
  setModal(true);
  ui.setupUi(this);
  ...
  ui.comboBoxEncoding->setCurrentIndex(
      ui.comboBoxEncoding->findText("ISO-8859-1", Qt::MatchExactly));
  connect(ui.comboBoxEncoding, SIGNAL(currentIndexChanged(const QString &)),
          this, SLOT(resetEditor()));
...
void UnicodeDialog::init(QFont &efont, bool line, QList<QColor> edcolors,
                         QList<QColor> hicolors) {
  unicodeviewWidget =
      new UnicodeView(ui.frameEditor, efont, line, edcolors, hicolors);
...
void UnicodeDialog::convertFile() {
  ...
  QTextCodec *codec =
      QTextCodec::codecForName(ui.comboBoxEncoding->currentText().toLatin1());
  // if(!codec) codec = QTextCodec::codecForLocale();
  unicodeviewWidget->editor->setPlainText(codec->toUnicode(file.readAll()));
}

void UnicodeDialog::resetEditor() { unicodeviewWidget->editor->clear(); }
