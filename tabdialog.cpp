...
TabDialog::TabDialog(QWidget *parent, const char *name) : QDialog(parent) {
  setWindowTitle(name);
  setModal(true);
  ui.setupUi(this);
  ...
  connect(ui.spinBoxColumns, SIGNAL(valueChanged(int)), this,
          SLOT(NewColumns(int)));
  ...
  connect(ui.checkBoxSpan, SIGNAL(toggled(bool)), this,
          SLOT(updateSpanStatus(bool)));

  connect(ui.pushButtonColumns, SIGNAL(clicked()), this,
          SLOT(applytoAllColumns()));
  connect(ui.pushButtonLines, SIGNAL(clicked()), this, SLOT(applytoAllLines()));

  connect(ui.spinBoxNumCol, SIGNAL(valueChanged(int)), this,
          SLOT(showColSettings(int)));
  connect(ui.spinBoxNumLi, SIGNAL(valueChanged(int)), this,
          SLOT(showRowSettings(int)));

  connect(ui.comboBoxColAl, SIGNAL(currentIndexChanged(int)), this,
          SLOT(updateColSettings()));
  connect(ui.comboLeftBorder, SIGNAL(currentIndexChanged(int)), this,
          SLOT(updateColSettings()));

  connect(ui.checkBoxBorderTop, SIGNAL(toggled(bool)), this,
          SLOT(updateRowSettings()));
  connect(ui.checkBoxSpan, SIGNAL(toggled(bool)), this,
          SLOT(updateRowSettings()));
  connect(ui.spinBoxSpanFrom, SIGNAL(valueChanged(int)), this,
          SLOT(updateRowSettings()));
  connect(ui.spinBoxSpanTo, SIGNAL(valueChanged(int)), this,
          SLOT(updateRowSettings()));

  connect(ui.comboBoxEndBorder, SIGNAL(currentIndexChanged(int)), this,
          SLOT(updateTableWidget()));

  connect(ui.tableWidget, SIGNAL(cellClicked(int, int)), this,
          SLOT(showColRowSettings(int, int)));
...
void TabDialog::updateColSettings() {
  ...
  colDataList.replace(i, col);
  updateTableWidget();
  // qDebug() << "change" << i << colDataList.at(i).alignment <<
  // colDataList.at(i).leftborder;
}
...
void TabDialog::showColSettings(int column) {
  ...
  if (i >= 99)
    return;
  disconnect(ui.comboBoxColAl, SIGNAL(currentIndexChanged(int)), this,
             SLOT(updateColSettings()));
  disconnect(ui.comboLeftBorder, SIGNAL(currentIndexChanged(int)), this,
             SLOT(updateColSettings()));
...
void TabDialog::showRowSettings(int row) {
  ...
  if (i >= 99)
    return;
  disconnect(ui.checkBoxBorderTop, SIGNAL(toggled(bool)), this,
             SLOT(updateRowSettings()));
  disconnect(ui.checkBoxSpan, SIGNAL(toggled(bool)), this,
             SLOT(updateRowSettings()));
  disconnect(ui.spinBoxSpanFrom, SIGNAL(valueChanged(int)), this,
             SLOT(updateRowSettings()));
  disconnect(ui.spinBoxSpanTo, SIGNAL(valueChanged(int)), this,
             SLOT(updateRowSettings()));
...
void TabDialog::updateTableWidget() {
  ...
  alignlist << QString("c") << QString("l") << QString("r") << QString("l p{}")
            << QString("c p{}") << QString("r p{}");
  ...
  for (int i = 0; i < y; i++) {
    for (int j = 0; j < x; j++) {
      ...
      else if (alignlist.at(colDataList.at(j).alignment).contains("r"))
      ...
      /*    if ((liDataList.at(i).merge) &&
         (liDataList.at(i).mergeto>liDataList.at(i).mergefrom)&&
         (j>=liDataList.at(i).mergefrom-1) && (j<=liDataList.at(i).mergeto-1))
            {
    ...
    if ((liDataList.at(i).merge) &&
        (liDataList.at(i).mergeto > liDataList.at(i).mergefrom)) {
      ui.tableWidget->setSpan(i, liDataList.at(i).mergefrom - 1, 1,
                              liDataList.at(i).mergeto -
                                  liDataList.at(i).mergefrom + 1);
    }
  }
}
