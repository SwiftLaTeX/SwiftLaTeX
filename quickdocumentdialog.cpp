...
void QuickDocumentDialog::Init() {
  ...
  if (!otherClassList.isEmpty())
    ui.comboBoxClass->addItems(otherClassList);
  ...
  if (!otherPaperList.isEmpty())
    ui.comboBoxPaper->addItems(otherPaperList);
  ...
  if (!otherEncodingList.isEmpty())
    ui.comboBoxEncoding->addItems(otherEncodingList);
  ...
  if (!otherOptionsList.isEmpty())
    ui.listWidgetOptions->addItems(otherOptionsList);
  ...
  if (!otherBabelList.isEmpty())
    ui.listWidgetBabel->addItems(otherBabelList);
}
