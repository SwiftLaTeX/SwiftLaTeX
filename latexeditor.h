...
public:
  ...
  int type;
  QTextCursor cursor;
  StructItem(int l, const QString &it, int t, const QTextCursor &curs)
      : line(l), item(it), type(t), cursor(curs){};
  bool operator==(const StructItem other) const {
    return ((item == other.item) && (type == other.type));
  }
  bool operator<(const StructItem other) const { return (item < other.item); }
};
...
class LatexEditor : public QPlainTextEdit {
  Q_OBJECT
public:
  LatexEditor(QWidget *parent, QFont &efont, QList<QColor> edcolors,
              QList<QColor> hicolors, bool inlinespelling = false,
              QString ignoredWords = "", Hunspell *spellChecker = 0,
              bool tabspaces = false, int tabwidth = 4,
              const QKeySequence viewfocus = QKeySequence("Ctrl+Space"),
              QString name = "", QStringList ulist = QStringList());
  ~LatexEditor();
  static void clearMarkerFormat(const QTextBlock &block, int markerId);
  void gotoLine(int line);
  bool search(const QString &expr, bool cs, bool wo, bool forward,
              bool startAtCursor, bool isRegExp);
  int searchInSelection(const QString &expr, bool cs, bool wo, bool isRegExp,
                        int start, int end);
  ...
  // const QRectF blockRect(const QTextBlock & block) {return
  // blockBoundingRect(block).translated(contentOffset());};
  ...
  void checkStructUpdate(QTextDocument *doc, int blockpos, QString text,
                         int line);
  ...
  QString selectedText() {
    return textCursor().selectedText().replace(QChar(QChar::ParagraphSeparator),
                                               "\n");
  }
  ...
  int selectionLength() const {
    return textCursor().selectionEnd() - textCursor().selectionStart();
  }
...
private:
  ...
  bool matchLeftLat(QTextBlock currentBlock, int index, int numRightLat,
                    int bpos);
  bool matchRightLat(QTextBlock currentBlock, int index, int numLeftLat,
                     int epos);
